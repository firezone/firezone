defmodule Portal.Repo.Migrations.CreateSentinelLogSinks do
  use Ecto.Migration
  ...
  def up do
    ...
    create(
      constraint(:log_sinks, :type_must_be_valid,
        check: "type IN ('splunk', 'datadog', 'newrelic', 'elastic', 'sentinel')"
      )
    )

    create table(:sentinel_log_sinks, primary_key: false) do
      ...
      add(:ingestion_endpoint, :text, null: false)
      add(:dcr_immutable_id, :string, null: false)
    ...
    create(unique_index(:sentinel_log_sinks, [:account_id, :name]))
  end

  def down do
    drop(table(:sentinel_log_sinks))
    drop(constraint(:log_sinks, :type_must_be_valid))
