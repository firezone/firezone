defmodule Portal.Repo.Migrations.MoveAutoCreateUsersToProviders do
  @moduledoc """
  ...
  defp oid_provider_keys do
    Portal.Repo.all(from("configurations", select: [:openid_connect_providers]))
  ...
  defp saml_provider_keys do
    Portal.Repo.all(from("configurations", select: [:saml_identity_providers]))
    # only one configuration at this point
    |> List.first()
  ...
  defp cur_oidc_create_users do
    Portal.Repo.all(from("configurations", select: [:auto_create_oidc_users]))
    |> List.first()
