defmodule Portal.Crl.Sync do
  ...
  defp apply_crl(endpoint, crl) do
    ...
    if superseded_only_by_delta?(endpoint, crl, delta) do
      skip_replace(endpoint, delta)
    else
      replace(endpoint, crl, delta)
    end
  end

  # Replacing the partition from a base we already hold, while this run's delta
  # is unavailable, would drop every revocation that only ever existed in the
  # delta. A newer base carries them itself, having been published after the
  # delta they came from, so only the unchanged-base case has to hold off.
  defp superseded_only_by_delta?(endpoint, crl, delta) do
    match?({:error, _reason}, delta) and not is_nil(crl.number) and
      crl.number == endpoint.crl_number
  end

  defp skip_replace(endpoint, delta) do
    Database.record_delta_error(endpoint, delta_error(delta))

    Logger.error("Keeping the cached revocation list: its delta is unavailable",
      account_id: endpoint.account_id,
      issuer: X509.describe_name(endpoint.issuer),
      distribution_point: endpoint.distribution_point,
      reason: inspect(delta_error(delta))
    )

    {:ok, :refreshed}
  end

  defp delta_error({:error, reason}), do: reason
  defp delta_error(_delta), do: nil

  defp replace(endpoint, crl, delta) do
  ...
  # A delta shares its issuer's numbering, so one worth applying names a base at
  # or below the list we hold and carries a higher number than it. A delta that
  # is older than the base would reintroduce revocations the base already
  # superseded, and removeFromCRL entries would un-revoke certificates the newer
  # base still lists.
  defp applies_to_this_list(delta, crl) do
    cond do
      ...
      is_nil(delta.number) -> {:error, :delta_number_missing}
      delta.number <= crl.number -> {:error, :delta_not_newer}
      true -> :ok
    end
  end
  ...
  defp address_fault?({:blocked_address, _message}), do: true
  ...
  defp fetch_one(url, endpoint, kind) do
    with :ok <- supported_scheme(url),
         ...
         {:ok, crl} <- decode(der),
         :ok <- covers_this_partition(crl, url, endpoint, kind),
         :ok <- fresher_than_cached(crl, endpoint, kind) do
      {:ok, crl}
    end
  end

  # A correctly signed list is still not one to act on if it predates what we
  # already hold. A stale mirror or a replayed response would otherwise roll the
  # cache back and un-revoke whatever the newer list had added.
  defp fresher_than_cached(crl, endpoint, :base) do
    cond do
      expired?(crl) -> {:error, :crl_expired}
      rolled_back?(crl.number, endpoint.crl_number) -> {:error, :crl_rolled_back}
      true -> :ok
    end
  end

  defp fresher_than_cached(crl, endpoint, :delta) do
    cond do
      expired?(crl) -> {:error, :crl_expired}
      rolled_back?(crl.number, endpoint.delta_number) -> {:error, :crl_rolled_back}
      true -> :ok
    end
  end

  defp expired?(%{next_update: nil}), do: false

  defp expired?(%{next_update: next_update}) do
    DateTime.compare(next_update, DateTime.utc_now()) != :gt
  end

  defp rolled_back?(nil, _cached), do: false
  defp rolled_back?(_number, nil), do: false
  defp rolled_back?(number, cached), do: number < cached
  ...
  defp covers_this_partition(crl, _url, endpoint, :base) do
    cond do
      crl.scope.delta? ->
        {:error, :crl_is_delta}
      ...
      not names_partition?(crl, endpoint.crl_urls) ->
  ...
  defp covers_this_partition(crl, url, endpoint, :delta) do
    cond do
      ...
      not names_partition?(crl, [url | endpoint.crl_urls]) ->
        {:error, :crl_wrong_partition}
  ...
  # RFC 5280 6.3.3 (b)(1) asks for any name in the list's Issuing Distribution
  # Point to match any name in the certificate's, rather than the one address
  # the fetch happened to use. A CA publishing to a CDN and an origin names only
  # one of them in the list itself, so comparing against the address we reached
  # would reject the list whenever the other one answered first.
  ...
  defmodule Database do
    ...
    # Only the delta fields move: the cached list itself is still the one we
    # fetched and verified, so its own freshness stamps stay as they are.
    def record_delta_error(endpoint, reason) do
      now = DateTime.utc_now()

      endpoint
      |> endpoint_query()
      |> update(set: [delta_fetched_at: ^now, delta_error: ^to_message(reason), updated_at: ^now])
      |> Safe.unscoped()
      |> Safe.update_all([])
    end
