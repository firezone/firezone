defmodule Portal.Policies.Postures.Checks do
  @moduledoc """
  Named posture checks, each a tree over the provider fields that answer one
  plain question such as "is the disk encrypted".

  A policy stores only the check's name. The tree here is expanded when the
  policy is parsed, so improving a mapping reaches every policy that uses it.
  Any provider that can answer the question satisfies the check.
  """
