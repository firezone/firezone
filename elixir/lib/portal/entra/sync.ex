defmodule Portal.Entra.Sync do
    ...
    case fetch_service_principal_id(directory, access_token, :directory_sync) do
      ...
      {:error, {:not_found, _response}} ->
        raise Entra.SyncError,
          error:
            {:consent_revoked,
             "Directory Sync app service principal not found in tenant. Admin consent may have been revoked. Please re-grant admin consent."},
          directory_id: directory.id,
  ...
  defp validate_assignments!(assignments, directory_id) do
    Enum.each(assignments, fn assignment ->
      unless assignment["principalId"] do
        raise Entra.SyncError,
          error: {:validation, "Assignment missing required 'principalId' field"},
      ...
      unless assignment["principalType"] do
        raise Entra.SyncError,
          error:
            {:validation,
             "Assignment '#{assignment["principalId"]}' missing required 'principalType' field"},
          directory_id: directory_id,
          step: :process_assignment
      end
      ...
      unless assignment["principalDisplayName"] do
        raise Entra.SyncError,
          error:
            {:validation,
             "Assignment '#{assignment["principalId"]}' missing required 'principalDisplayName' field"},
          directory_id: directory_id,
          step: :process_assignment
      end
    end)
  ...
  defp process_assigned_group_members_page(directory, synced_at, group_id, group_name, members) do
    ...
    Enum.each(user_members, fn member ->
      unless member["id"] do
        raise Entra.SyncError,
          error: {:validation, "User member missing required 'id' field in group #{group_name}"},
          directory_id: directory.id,
      ...
      groups when is_list(groups) ->
        ...
        Enum.each(groups, fn group ->
          ...
          unless group["displayName"] do
            raise Entra.SyncError,
              error: {:validation, "Group '#{group["id"]}' missing required 'displayName' field"},
              directory_id: directory.id,
              step: :process_group
          end
        end)
  ...
  defp map_user_to_identity(user, directory_id) do
    ...
    unless primary_email do
      raise Entra.SyncError,
        error:
          {:validation, "User '#{user["id"]}' missing both 'mail' and 'userPrincipalName' fields"},
        directory_id: directory_id,
        step: :process_user
    end
