defmodule Portal.Dev.AccountPopulation do
    ...
    gateway =
      ...
      |> cast(
        %{
          ...
          external_id: "#{state.account.slug}-gw-#{site.id}-#{index}"
        },
        [:account_id, :site_id, :name, :external_id]
      )
      |> Gateway.changeset()
      |> Repo.insert!()

    {:ok, _} =
      IPv4Address.allocate_next_available_address(state.account.id, gateway_id: gateway.id)

    {:ok, _} =
      IPv6Address.allocate_next_available_address(state.account.id, gateway_id: gateway.id)
  ...
  defp create_client(state, actor, index, verified?) do
    ...
    client =
      ...
      |> cast(
        %{
          ...
          actor_id: actor.id,
          name: "Client #{index}",
          external_id: "#{state.account.slug}-client-#{actor.id}-#{index}",
    ...
    {:ok, _} = IPv4Address.allocate_next_available_address(state.account.id, client_id: client.id)
    {:ok, _} = IPv6Address.allocate_next_available_address(state.account.id, client_id: client.id)
  ...
  defp build_summary(account, plan, level) do
    ...
    counts = %{
      ...
      gateways: count_records(Gateway, account.id),
      clients: count_records(Client, account.id)
    }
