defmodule Portal.Changes.Hooks.SentinelOneDevices do
  @moduledoc """
  Hooks for the device rows a SentinelOne sync writes.
  ...
  def on_insert(lsn, data) do
    device = struct_from_params(Portal.SentinelOne.Device, data)
    change = %Change{lsn: lsn, op: :insert, struct: device}

    PubSub.Changes.broadcast(device.account_id, :sentinelone_devices, change)
  end
  ...
  def on_update(lsn, old_data, data) do
    if bookkeeping_only_change?(old_data, data) do
      ...
      old_device = struct_from_params(Portal.SentinelOne.Device, old_data)
      device = struct_from_params(Portal.SentinelOne.Device, data)
  ...
  def on_delete(lsn, old_data) do
    device = struct_from_params(Portal.SentinelOne.Device, old_data)
