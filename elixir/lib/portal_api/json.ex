defmodule PortalAPI.JSON do
  ...
  through `PortalAPI.JSON.Encoder` and wrapped in the `data` envelope.
      ...
      json(conn, JSON.encode(gateway, schema: Gateway.Schema, token: secret))
  ...
  a module or a function of the struct. Any other option is a value the request
  produced and the struct does not carry, added to the encoded data.
  """
  ...
  def encode(struct, opts) when is_struct(struct) and is_list(opts) do
    ...
    %{data: struct |> encode_with(schema) |> Map.merge(Map.new(extras))}
  end
