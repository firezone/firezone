defmodule PortalWeb.CoreComponents do
    ...
    <div id={@id} class="relative" phx-hook="CopyClipboard">
      ...
      >
        <span id={"#{@id}-default-message"} class="inline-flex items-center">
          <.icon name="remix-clipboard-line" data-icon class="h-4 w-4 me-1.5" />
          <span class="text-xs font-semibold">Copy</span>
        </span>
        <span id={"#{@id}-success-message"} class="hidden items-center">
          <.icon name="remix-check-line" data-icon class="text-green-700 h-4 w-4 me-1.5" />
  ...
  def copy(assigns) do
    ...
    <div id={@id} class={@class} phx-hook="CopyClipboard" {@rest}>
      ...
      >
        <.icon name="remix-clipboard-line" data-icon class="h-4 w-4" />
      </button>
    </div>
    """
  end
  ...
  def flash(assigns) do
    ...
    >
      <div class={["flex items-start gap-2", @style == "toast" && ""]}>
        <.icon
          :if={@base_kind == :info}
          name="remix-information-line"
          class="h-4 w-4 shrink-0 mt-0.5"
        />
        <.icon
          :if={@base_kind == :success}
          name="remix-checkbox-circle-line"
          class="h-4 w-4 shrink-0 mt-0.5"
        />
        <.icon
          :if={@base_kind == :warning}
          name="remix-error-warning-line"
          class="h-4 w-4 shrink-0 mt-0.5"
        />
        <.icon
          :if={@base_kind == :error}
  ...
  def error(assigns) do
    ...
    <p
      ...
      {@rest}
    >
      <.icon name="remix-alert-line" class="h-4 w-4 flex-none" />
      {render_slot(@inner_block)}
    </p>
    """
  end
  ...
  def base_error(assigns) do
    ...
    <p
      ...
      {@rest}
    >
      <.icon name="remix-alert-line" class="mt-0.5 h-5 w-5 flex-none" />
  ...
  ## Examples
      ...
      <.icon name="remix-close-fill" />
      <.icon name="remix-loop-left-line" class="ml-1 w-3 h-3 animate-spin" />
      <.icon name="remix-user-fill" class="w-5 h-5" />
      <.icon name="remix-settings-3-line" class="w-4 h-4 text-gray-500" />
  """
  ...
  def initial_connection_status(assigns) do
    ...
    >
      ...
      <span :if={@connected?}>
        <.icon name="remix-check-line" class="h-3.5 w-3.5 mr-1" /> Connected, click to continue
      </span>
    </.link>
    """
  end
  ...
  def verified(%{schema: %{verified_at: _verified_at}} = assigns) do
    ...
    <div class="flex items-center gap-x-1">
      <.icon name="remix-shield-check-line" class="w-4 h-4" /> Verified
  ...
  def group_badge(%{group: nil} = assigns) do
    ...
    <span class={[
      ...
      <span class="inline-flex items-center justify-center py-0.5 px-1.5 text-primary-600 bg-primary-100 border-r border-primary-300">
        <.icon name="remix-error-warning-line" class="h-3.5 w-3.5" />
      </span>
  ...
  def last_seen(assigns) do
    ...
    <%= if @schema do %>
      ...
      <span class="inline-block">
        ...
        >
          <.icon name="remix-external-link-line" class="mb-3 w-3 h-3" />
        </a>
      </span>
    <% else %>
  ...
  def provider_icon(%{type: "email_otp"} = assigns) do
    ~H"""
    <.icon name="remix-mail-line" {@rest} />
    """
  end
  ...
  def provider_icon(%{type: "userpass"} = assigns) do
    ~H"""
    <.icon name="remix-key-line" {@rest} />
    """
  end

  def provider_icon(%{type: "unknown"} = assigns) do
    ~H"""
    <.icon name="remix-question-line" {@rest} />
    """
  end
