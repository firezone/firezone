defmodule PortalWeb.Settings.Authentication do
    ...
    <div class="flex flex-col h-full">
      ...
      <div class="flex-1 flex flex-col overflow-hidden">
        <div class="flex items-center justify-between px-6 py-3 border-b border-[var(--border)] shrink-0">
          ...
          <.link
            patch={~p"/#{@account}/settings/authentication/select_type"}
  ...
  defp provider_row(assigns) do
    ...
    is_default = Map.has_key?(provider, :is_default) && provider.is_default
    can_be_default = type not in ["email_otp", "userpass"]
    can_be_deleted = type not in ["email_otp", "userpass"]
    has_sessions = provider.client_tokens_count > 0 or provider.portal_sessions_count > 0
    is_pending_toggle = pending && pending.id == provider.id && pending.action == "toggle"
    is_pending_delete = pending && pending.id == provider.id && pending.action == "delete"
    is_pending_revoke = pending && pending.id == provider.id && pending.action == "revoke"
    ...
    assigns =
      assign(assigns,
        ...
        is_pending_toggle: is_pending_toggle,
        is_pending_delete: is_pending_delete,
        is_pending_revoke: is_pending_revoke,
        portal_ttl: portal_ttl,
                  ...
                  <:content>
                    <div class="flex flex-col py-1 w-44">
                      ...
                      >
                        <.icon
                          name={
                            if @provider.is_disabled, do: "remix-checkbox-circle-line", else: "remix-close-circle-line"
                          }
