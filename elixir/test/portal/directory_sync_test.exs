defmodule Portal.DirectorySyncTest do
  ...
  setup do
    ...
    %{
      ...
      directory: directory,
      issuer: issuer,
      long_ago: DateTime.add(now, -7200, :second),
  ...
  describe "identities" do
    test "a full sync page from before a webhook removal cannot bring the user back", ctx do
      identity = directory_identity(ctx, "user-1", ctx.long_ago)

      assert {1, nil} = remove_identity(ctx, "user-1", ctx.webhook_at)
    ...
    test "a removal leaves a tombstone even when the directory never had the user", ctx do
      assert {0, nil} = remove_identity(ctx, "user-1", ctx.webhook_at)
      assert state(IdentityState, ctx, "user-1")
    ...
    test "a removal deletes whatever row currently holds the key", ctx do
      stale = directory_identity(ctx, "user-1", ctx.long_ago)
      Repo.delete!(stale)

      assert {:ok, %{upserted_identities: 1}} =
               upsert_identities(ctx, ctx.sync_start, [identity_attrs("user-1")])

      recreated = Repo.get_by!(ExternalIdentity, idp_id: "user-1", account_id: ctx.account.id)
      refute recreated.id == stale.id

      assert {1, nil} = remove_identity(ctx, "user-1", ctx.webhook_at)
      refute Repo.get_by(ExternalIdentity, id: recreated.id)
    end
    ...
    test "the cleanup removes users no listing confirmed during the run", ctx do
      refreshed = directory_identity(ctx, "refreshed", ctx.long_ago)
      confirmed = directory_identity(ctx, "confirmed", ctx.long_ago)

      assert {:ok, _} =
               upsert_identities(ctx, ctx.webhook_at, [identity_attrs("refreshed")],
                 eligible: false
               )

      assert {:ok, _} = upsert_identities(ctx, ctx.webhook_at, [identity_attrs("confirmed")])
      ...
      refute Repo.get_by(ExternalIdentity, id: refreshed.id)
      assert Repo.get_by(ExternalIdentity, id: confirmed.id)

      tombstone = state(IdentityState, ctx, "refreshed")
      assert DateTime.compare(tombstone.synced_at, ctx.webhook_at) == :eq
    end
    ...
    test "the cleanup leaves a tombstone that blocks writes from before the run", ctx do
      ...
      assert {:ok, %{upserted_identities: 0}} =
               upsert_identities(ctx, DateTime.add(ctx.sync_start, -1, :second), [
    ...
    test "a group listing cannot revive a tombstone written moments ago", ctx do
      directory_identity(ctx, "user-1", ctx.long_ago)
      assert {1, nil} = remove_identity(ctx, "user-1", ctx.webhook_at)

      assert {:ok, %{upserted_identities: 0}} =
               upsert_identities(ctx, ctx.later, [identity_attrs("user-1")], quarantine: true)

      assert {:ok, %{upserted_identities: 1}} =
               upsert_identities(
                 ctx,
                 DateTime.add(ctx.webhook_at, 3600, :second),
                 [identity_attrs("user-1")],
                 quarantine: true
               )
    end
  ...
  describe "groups" do
    test "a full sync page from before a webhook removal cannot bring the group back", ctx do
      group = directory_group(ctx, "group-1", ctx.long_ago)

      assert {1, nil} =
               DirectorySync.remove_group(
                 ctx.account.id,
                 ctx.directory.id,
                 "group-1",
                 ctx.webhook_at
      ...
      assert {:ok, %{upserted_groups: 0}} = upsert_groups(ctx, ctx.sync_start, "group-1")
      refute Repo.get_by(Group, idp_id: "group-1", account_id: ctx.account.id)

      assert {:ok, %{upserted_groups: 1}} = upsert_groups(ctx, ctx.later, "group-1")
      assert Repo.get_by(Group, idp_id: "group-1", account_id: ctx.account.id)
    end

    test "a removal fences a group the directory never inserted", ctx do
      assert {0, nil} =
               DirectorySync.remove_group(
                 ctx.account.id,
                 ctx.directory.id,
  ...
  describe "memberships" do
    test "a group member list rewrite blocks membership writes from before it", ctx do
      directory_group(ctx, "group-1", ctx.long_ago)
      identity = directory_identity(ctx, "user-1", ctx.long_ago)

      assert {:ok, _} = upsert_groups(ctx, ctx.webhook_at, "group-1")
    ...
    test "an org unit rewrite blocks only older org unit membership writes", ctx do
      directory_group(ctx, "group-1", ctx.long_ago)
      ...
      org_unit_group_fixture(
        ...
        idp_id: "ou-1",
        synced_at: ctx.long_ago
      )

      identity = directory_identity(ctx, "user-1", ctx.long_ago)

      assert {1, nil} =
               DirectorySync.stamp_identity_org_unit_memberships(
                 ctx.account.id,
                 ctx.directory.id,
                 "user-1",
                 ctx.webhook_at
      ...
      assert {:ok, %{upserted_memberships: 1}} =
               upsert_memberships(ctx, ctx.sync_start, [{"ou-1", "user-1"}, {"group-1", "user-1"}])

      group = Repo.get_by!(Group, idp_id: "group-1", account_id: ctx.account.id)
      assert Repo.get_by(Membership, actor_id: identity.actor_id, group_id: group.id)

      org_unit = Repo.get_by!(Group, idp_id: "ou-1", account_id: ctx.account.id)
      refute Repo.get_by(Membership, actor_id: identity.actor_id, group_id: org_unit.id)
    end
    ...
    test "duplicate input tuples collapse into one membership", ctx do
      directory_group(ctx, "group-1", ctx.long_ago)
      identity = directory_identity(ctx, "user-1", ctx.long_ago)

      assert {:ok, %{upserted_memberships: 1}} =
               upsert_memberships(ctx, ctx.webhook_at, [
                 {"group-1", "user-1"},
                 {"group-1", "user-1"}
               ])
  ...
  describe "prune_tombstones/3" do
    test "drops dead state rows older than the previous run and the grace period", ctx do
      live = directory_identity(ctx, "live", ctx.long_ago)
      previous_run = DateTime.add(ctx.webhook_at, -10, :second)

      tombstone(IdentityState, ctx, "old-dead", DateTime.add(ctx.webhook_at, -7200, :second))
      tombstone(IdentityState, ctx, "recent-dead", DateTime.add(ctx.webhook_at, -600, :second))
      tombstone(GroupState, ctx, "old-group", DateTime.add(ctx.webhook_at, -7200, :second))

      assert :ok = DirectorySync.prune_tombstones(ctx.account.id, ctx.directory.id, previous_run)

      assert state(IdentityState, ctx, live.idp_id)
      refute Repo.get_by(IdentityState, directory_id: ctx.directory.id, idp_id: "old-dead")
      assert state(IdentityState, ctx, "recent-dead")
  ...
  defp upsert_identities(ctx, synced_at, attrs, opts \\ []) do
  ...
  defp upsert_groups(ctx, synced_at, idp_id) do
    DirectorySync.batch_upsert_groups(
      ctx.account.id,
      ctx.directory.id,
      synced_at,
      [%{idp_id: idp_id, name: idp_id}],
      :group
    )
  end
  ...
  defp remove_identity(ctx, idp_id, synced_at) do
    DirectorySync.remove_identity(ctx.account.id, ctx.directory.id, ctx.issuer, idp_id, synced_at)
  end
