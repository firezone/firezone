defmodule Portal.Google.SyncErrorTest do
  use ExUnit.Case, async: true

  alias Portal.Google.SyncError
  ...
  describe "exception/1" do
    test "creates exception with string reason" do
      ...
      assert exception.message =~ "Google sync failed"
    ...
    test "builds message from HTTP response-like cause" do
      ...
      assert exception.message =~ @test_directory_id
      assert exception.message =~ "stream_users"
    end
    ...
    test "stores all fields correctly" do
      ...
      assert exception.step == :sync_org_units
    ...
    test "message includes all context when raised" do
      try do
        ...
        e in SyncError ->
          ...
          assert message =~ "Google sync failed"
  ...
  describe "message building for different error types" do
    test "formats string reason cleanly" do
      ...
      assert exception.message ==
               "Google sync failed for directory #{@test_directory_id} at fetch_user: User not found"
    end
    ...
    test "formats HTTP response with status and body" do
      ...
      assert message =~ "Google sync failed for directory #{@test_directory_id}"
