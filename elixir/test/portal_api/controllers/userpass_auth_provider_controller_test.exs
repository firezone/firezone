defmodule PortalAPI.UserpassAuthProviderControllerTest do
  ...
  describe "index/2" do
    test "returns error when not authorized", %{conn: conn} do
      conn = get(conn, "/userpass_auth_providers")
      assert json_response(conn, 401) == %{"error" => %{"reason" => "Unauthorized"}}
    end

    test "lists all userpass auth providers", %{conn: conn, account: account, actor: actor} do
    ...
    test "only lists providers from the authorized account", %{
      ...
      other_provider = userpass_provider_fixture()
  ...
  describe "show/2" do
    test "returns error when not authorized", %{conn: conn, account: account} do
      provider = userpass_provider_fixture(account: account)
      conn = get(conn, "/userpass_auth_providers/#{provider.id}")
      assert json_response(conn, 401) == %{"error" => %{"reason" => "Unauthorized"}}
    end

    test "shows a userpass auth provider", %{conn: conn, account: account, actor: actor} do
      provider = userpass_provider_fixture(account: account)
      ...
      conn =
        ...
        |> authorize_conn(actor)
        |> put_req_header("content-type", "application/json")
        |> get("/userpass_auth_providers/#{provider.id}")
    ...
    test "returns not found for unknown id", %{conn: conn, actor: actor} do
      conn =
        ...
        |> authorize_conn(actor)
        |> put_req_header("content-type", "application/json")
        |> get("/userpass_auth_providers/#{Ecto.UUID.generate()}")
