defmodule Web.Auth.SettingsLive.IdentityProviders.OpenIDConnect.Connect do
  ...
  describe "redirect_to_idp/2" do
    ...
    test "redirects to IdP when provider exists", %{
      ...
      conn =
        ...
        |> assign(:account, account)
        |> get(
          ~p"/#{account.id}/settings/identity_providers/openid_connect/#{provider.id}/redirect",
          %{}
        )
  ...
  describe "handle_idp_callback/2" do
    ...
    test "redirects with an error when state cookie does not exist", %{
      ...
      conn =
        ...
        |> assign(:account, account)
        |> get(
          ~p"/#{account}/settings/identity_providers/openid_connect/#{provider}/handle_callback",
          %{
            "state" => "XOXOX",
            "code" => "bar"
          }
      ...
      assert redirected_to(conn) ==
               ~p"/#{account}/settings/identity_providers/openid_connect/#{provider}"
    ...
    test "redirects to the dashboard when credentials are valid and return path is empty", %{
      ...
      assert provider.adapter_state == %{"status" => "connected"}
