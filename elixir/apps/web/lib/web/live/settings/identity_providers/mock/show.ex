defmodule Web.Settings.IdentityProviders.Mock.Show do
  use Web, :live_view
  ...
  def render(assigns) do
    ...
    <.breadcrumbs account={@account}>
      ...
      <.breadcrumb path={~p"/#{@account}/settings/identity_providers/mock/#{@provider}"}>
        {@provider.name}
      </.breadcrumb>
    </.breadcrumbs>
        ...
        <div class="bg-white overflow-hidden">
          <.vertical_table id="provider">
            ...
            <.vertical_table_row>
              ...
              <:value>
                The Mock Identity Provider is a test provider that generates random Actors and
                Groups using the same sync mechanism as the other directory sync adapters. The
                difference is that it performs no actual network requests and generates random
                data.
              </:value>
            </.vertical_table_row>
            <.vertical_table_row>
            ...
            <.vertical_table_row>
              ...
              <:value>{@provider.adapter_config["num_actors"]}</:value>
            </.vertical_table_row>
            <.vertical_table_row>
              <:label>Number of groups to generate</:label>
              <:value>{@provider.adapter_config["num_groups"]}</:value>
            </.vertical_table_row>
            <.vertical_table_row>
              <:label>Max number of actors per group</:label>
              <:value>{@provider.adapter_config["max_actors_per_group"]}</:value>
            </.vertical_table_row>
            <.vertical_table_row>
              <:label>Created</:label>
              <:value>
  ...
  def handle_event("delete", _params, socket) do
    :ok = Auth.delete_provider(socket.assigns.provider, socket.assigns.subject)
