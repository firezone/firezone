defmodule Domain.Auth do
  ...
  def all_providers_pending_sync_by_adapter!(adapter) do
    ...
    |> Provider.Query.by_adapter(adapter)
    |> Provider.Query.by_provisioner(:custom)
    |> Provider.Query.has_no_associated_directory()
  ...
  # TODO: IdP sync
  # Move to a new identities context in https://github.com/firezone/firezone/issues/10338
  # Used for new sync implementation
  def batch_upsert_identities_with_actors(%Provider{} = provider, attrs_list, synced_at) do
    ...
    existing_identities =
      ...
      |> Identity.Query.by_account_id(provider.account_id)
      |> Identity.Query.by_provider_id(provider.id)
      |> Identity.Query.by_provider_identifier({:in, provider_identifiers})
      |> Repo.all()
      |> Enum.into(%{}, fn identity ->
        {identity.provider_identifier, identity}
      end)

    {to_insert, to_update} =
      Enum.split_with(attrs_list, fn identity ->
        not Map.has_key?(existing_identities, identity["provider_identifier"])
      end)

    {new_actors, actor_id_mapping} =
      to_insert
      |> Enum.map(fn identity ->
        ...
        {actor, {identity["provider_identifier"], actor_id}}
      end)
      |> Enum.unzip()

    {_count, _inserted_actors} = Repo.insert_all(Actors.Actor, new_actors, returning: false)
    provider_to_actor_id = Map.new(actor_id_mapping)

    identities_to_insert =
      to_insert
      |> Enum.map(fn identity ->
        email = get_in(identity, ["provider_state", "userinfo", "email"])
        actor_id = provider_to_actor_id[identity["provider_identifier"]]
        ...
        %{
          ...
          provider_state: identity["provider_state"],
          created_by: :provider,
    ...
    {_count, inserted_identities} =
      Repo.insert_all(Identity, identities_to_insert,
        returning: [:id, :actor_id, :provider_identifier]
      )

    actor_updates =
      to_update
      |> Enum.map(fn identity ->
        existing_identity = existing_identities[identity["provider_identifier"]]

        {existing_identity.actor_id, identity["actor"]["name"]}
      end)
    ...
    |> Enum.each(fn batch ->
      placeholders =
        batch
        |> Enum.with_index()
        |> Enum.map(fn {{_actor_id, _name}, index} ->
          # skip first two params
          param_id = index * 2 + 3
          param_name = param_id + 1

          "($#{param_id}::uuid, $#{param_name})"
        end)
        |> Enum.join(", ")

      params =
        [synced_at, now] ++
          Enum.flat_map(batch, fn {actor_id, name} -> [Ecto.UUID.dump!(actor_id), name] end)
      ...
      sql = """
        ...
        SET name = updates.name,
            last_synced_at = $1,
            updated_at = $2
        FROM (VALUES #{placeholders}) AS updates(id, name)
        WHERE actors.id = updates.id
      """
    ...
    identities_to_update =
      ...
      |> Enum.map(fn identity ->
        ...
        %{
          ...
          actor_id: existing_identity.actor_id,
          account_id: provider.account_id,
          provider_id: provider.id,
          email: email,
          provider_identifier: identity["provider_identifier"],
    ...
    {_count, updated_identities} =
      Repo.insert_all(
        Identity,
        identities_to_update,
        on_conflict: {:replace, [:provider_state, :email, :synced_at]},
        conflict_target: :id,
        returning: [:id, :actor_id, :provider_identifier]
      )

    {:ok, inserted_identities ++ updated_identities}
  end
