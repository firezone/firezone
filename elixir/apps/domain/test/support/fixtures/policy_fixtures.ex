defmodule Domain.PolicyFixtures do
  @moduledoc """
    ...
    account =
      cond do
        group = Map.get(attrs, :group) ->
          group.account || Domain.Repo.preload(group, :account).account
  ...
  def group_policies_fixture(group, resource_count \\ 3, attrs \\ %{}) do
    account = group.account || Domain.Repo.preload(group, :account).account
  ...
  def resource_policies_fixture(resource, group_count \\ 3, attrs \\ %{}) do
    account = resource.account || Domain.Repo.preload(resource, :account).account
