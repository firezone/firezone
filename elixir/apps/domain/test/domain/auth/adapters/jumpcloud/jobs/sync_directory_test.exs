defmodule Domain.Auth.Adapters.JumpCloud.Jobs.SyncDirectoryTest do
  ...
  describe "execute/1" do
    ...
    test "syncs IdP data", %{provider: provider} do
      ...
      groups =
        [
          %{"id" => "GROUP_ALL_ID", "name" => "All"},
          %{"id" => "GROUP_ENGINEERING_ID", "name" => "Engineering"}
        ]
        |> Enum.map(fn group ->
          %{
            "attributes" => nil,
      ...
      JumpCloudDirectory.mock_groups_list_endpoint(bypass, groups)
      JumpCloudDirectory.mock_users_list_endpoint(bypass, users)

      Enum.each(groups, fn group ->
        JumpCloudDirectory.mock_group_members_list_endpoint(bypass, group["id"], members)
      end)
    ...
    test "updates existing groups and memberships", %{account: account, provider: provider} do
      ...
      JumpCloudDirectory.mock_group_members_list_endpoint(
        bypass,
        "GROUP_ALL_ID",
        two_members
      )
    ...
    test "stops the sync retires on 401 error on the provider", %{provider: provider} do
      ...
      error_message = "Unauthorized: api key user not found"

      response = %{
        "message" => "Unauthorized: api key user not found",
        "error" => "Unauthorized: api key user not found"
      }
    ...
    test "persists the sync error on the provider", %{provider: provider} do
      ...
      refute updated_provider.last_synced_at
      assert updated_provider.last_syncs_failed == 1
      assert updated_provider.last_sync_error == "JumpCloud API is temporarily unavailable"
