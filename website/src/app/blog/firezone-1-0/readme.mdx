...
It was born out of necessity &mdash; as an engineer at Cisco I found myself in
need of an easy way to deploy and manage a VPN server for security automation. I
had used OpenVPN in the past and loathed it, so this time I decided to try a
fast, new contender called [WireGuard](https://www.wireguard.com).
...
were worth the price of admission, colleagues and I agreed &mdash; _wouldn't it
be great if a tool existed to do this for us?_
...
So let's back up &mdash; what is a VPN anyway, and why is one needed at all? To
...
&mdash; connecting to the Internet was expensive, after all. Typically only
banks, universities, and other large institutions could justify the cost.
...
Soon, all types of entities wanted to connect &mdash; local governments,
schools, small businesses. Internet Service Providers began offering connections
to mere individuals. Eventually there were so many entities on the Internet that
...
perimeter we had before vanishes &mdash; inside and outside entities are equally
untrusted.
...
untrusted entity. But wait, hold on a minute &mdash; what happened to the
firewall?
...
and this technique would still work &mdash; the principles are the same.
