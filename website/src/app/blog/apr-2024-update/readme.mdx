...
you to try it. [Sign up](https://app.firezone.dev/sign_up) now to get started.
...
So we went back to the whiteboard and imagined what Firezone would look like if
...
adddress handed out by an Erlang-powered telecom switch.
...
manage acccess policies that don't require a PhD in network security.

Firezone 1.0 delivers on all of that and more.

#### High availability

One major feature in 1.0 we want to mention specifically is high availability.
...
in VM, a container, on an IoT device -- it's lightweight enough to run
...
into that Site as you'd like. If you do, all Gateways in the Site will work in
unison to provide load balancing and automatic failover for all connections to
...
This happens completely transparently to the user and in most cases within a few
seconds.

Armed with this ability, admins can now enjoy a simple simple maintenance
process: (1) take the Gateway down, (2) upgrade it, and (3) bring it back up.
_That's it_. No more lengthy maintenance windows, backing up configurations, or
worrying about extended downtime.
...
More precisely, configuring your organizations' cloud or corporate firewals to
...
single firewall configuration still enjoy the same level of performance as if
you did. Attack surface is minimized and connections are direct. It's a win-win.
...
Firezone today supports directory sync with [Okta](https://www.okta.com/),
[Entra ID](https://azure.microsoft.com/en-us/services/active-directory/), and
[Google Workspace](https://workspace.google.com/) and more on on the way.
