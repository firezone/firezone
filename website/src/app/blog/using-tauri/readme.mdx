...
apps, so the Rust code is isolated in `src-tauri`, and the project is
...
This isn't Tauri's fault, it's a direct effect of using web technology.

The GUI process consistently uses about 100 MB of RAM even when idle. This seems
to be the minimum for keeping the web view process alive. Even if we hide all windows, it
won't reduce the amount of RAM.

We might be able to fix this in the future by closing the windows instead of
just hiding them, and at least we're theoretically able to share code pages with
other web view processes, we're not loading our own copy of Chromium like
Electron does.
...
Not Tauri's fault. Ubuntu 20.04 ships `webkit2gtk` version 4.0. Ubuntu 22.04
ships version 4.0 and 4.1. Ubuntu 24.04 only ships 4.1.

It's not possible to link with both, so it's theoretically impossible to make
one exe that runs on all 3 of the Ubuntu versions we'd like to support. For now
we are not supporting 24.04 yet, and in the future we may give it a separate
package. This is common in Linux programs that use anything more than the most
common libraries.
...
times out in our GitHub CI runners and it isn't clear why. Not Tauri's fault.
