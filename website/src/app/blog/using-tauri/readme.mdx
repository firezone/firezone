...
> as nice to program as CLIs. Oh well.
...
fast binaries for all major desktop platforms."
...
Tauri does the same thing, shipping the `create-tauri-app` tool for setting up
...
{/_ flowchart TD A(Firezone Client) --> B(Tauri) B --> C(WRY) C --> D("WebView2
(Windows)") C --> E("webkit2gtk (Linux)") C --> F(TAO) B --> G(Tokio) A --> G A
--> J(connlib) F --> H(GTK+) F --> I(Windows) _/}

<Link target="_blank" href="/images/blog/using-tauri/tauri-arch.svg">
  <Image
    src="/images/blog/using-tauri/tauri-arch.svg"
    alt="Tauri dependency diagram"
    width={1200}
    height={1200}
    className="p-16"
  />
...
Tauri is built atop [WRY](https://github.com/tauri-apps/wry), the webview
rendering library. WRY is built atop [TAO](https://github.com/tauri-apps/tao),
the window creation library. All 3 projects are maintained by the Tauri
Programme.

WRY in turn uses platform-specific web views like `webkit2gtk` on Linux and
[WebView2](https://developer.microsoft.com/en-us/microsoft-edge/webview2) on
Windows. TAO creates windows using GTK+ on Linux and the Windows API on Windows.

The Firezone Client sits above all that and bundles our headless connection
library `connlib` into a Tauri app. Tauri and Firezone both use the async
runtime [Tokio](https://github.com/tokio-rs/tokio).

The release packages are small because we don't take responsibility for
installing WebView2 or `webkit2gtk`. Windows 11 and some Windows 10 systems have
WebView2 pre-installed. Tauri automatically runs a downloader when the MSI is
installed if needed. On Linux (Debian and Ubuntu), we tell `apt` that we depend
on `webkit2gtk`, so `apt` will install it for us.

{/_ flowchart TD A(MSI) --> B(GUI exe) A --> C(IPC service exe) A --> D(WebView2
downloader) _/}

<Link target="_blank" href="/images/blog/using-tauri/msi-bundle.svg">
  <Image
    src="/images/blog/using-tauri/msi-bundle.svg"
    alt="MSI bundle diagram"
    width={1200}
    height={1200}
    className="p-16"
  />
...
{/_ flowchart TD A(deb) --> B(GUI exe) A --> C(IPC service exe) A -->
D("webkit2gtk dependencies") _/}

<Link target="_blank" href="/images/blog/using-tauri/deb-bundle.svg">
  <Image
    src="/images/blog/using-tauri/deb-bundle.svg"
    alt="deb bundle diagram"
    width={1200}
    height={1200}
    className="p-16"
  />
...
just doesn't seem to work, and we need root permissions to control DNS.
...
- A GUI running as the normal user, and
- A systemd service running as root, with lots of sandboxing.

This puts a boundary between the millions of lines of WebKit and JavascriptCore
code in the GUI, and the cryptography and connection code in `connlib`, so it
may improve security. It also allows non-sudo users to run the GUI, and it
allows the tunnel service to clean up the connection even if the GUI panics or
crashes.
