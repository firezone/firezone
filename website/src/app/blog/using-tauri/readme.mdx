...
_The Windows Client, at time of writing_
...
- Controlling DNS and running the Firezone tunnel requires admin / sudo
  ...
  that service takes commands from the GUI. Installing a service from a portable
  executable makes it not portable.
...
the download size from ~10 MB to ~100 MB, we can't install our tunnel service if
we're running from an AppImage, and AppImages built on 20.04 have rendering bugs
on 24.04 anyway.
...
to generate an MSI that installs and configures our tunnel service. So in both
