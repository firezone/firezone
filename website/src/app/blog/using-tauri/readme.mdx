...
So WebView2 is an API for Edge, which is Chromium, which is Blink, which was
WebKit, which was KHTML.

Therefore, Tauri is just [Konqueror](https://en.wikipedia.org/wiki/Konqueror),
the KDE2 web browser.

TODO: Do we have permissions to rehost
https://en.wikipedia.org/wiki/K_Desktop_Environment_2#/media/File:KDE_2.2.2.png
with the caption "The primordial soup from which the Web evolved"?
...
So the final word is, **we like Tauri a lot**.
