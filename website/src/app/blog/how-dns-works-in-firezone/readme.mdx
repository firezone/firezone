...
requests a gateway serving that resource to resolve the DNS query. When the gateway
responds, the proxy will create a temporary mapping between the actual IP
address of the resource and the dummy IP address it returned to the application.
From this point onward, all subsequent packets to the dummy IP address are
forwarded to the gateway that resolved the query. These IPs are persisted 
