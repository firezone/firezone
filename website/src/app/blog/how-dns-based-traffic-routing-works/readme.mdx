...
created to each host's IP address to a human-friendly "hostname".
...
of nameservers, each one responsible for a resolving different part.
...
nameservers again. This can speed up query times orders of magnitude, to the
...
finally it needs to be reasonably sure these IP addresses don't conflict with
...
and the shortcomings of Split DNS, into a single solution that's just as secure
