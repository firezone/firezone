...
function CopyButton({ value }: { value: string }) {
  ...
  return (
    <button
      ...
      className="flex-none p-2 rounded-sm text-neutral-500 hover:text-neutral-200 hover:bg-neutral-600 transition duration-200 cursor-pointer"
      aria-label="Copy to clipboard"
    >
      {copied ? (
        <HiOutlineCheck className="h-4 w-4 text-accent-400" />
      ) : (
...
export default function Page() {
  return (
    <div className="flex flex-col">
      ...
      >
        <h2 className="tracking-tight text-2xl md:text-3xl lg:text-4xl font-semibold mb-4">
          <HiOutlineWrenchScrewdriver className="w-8 h-8 mr-4" />
          Diagnostic report
        </h2>
        <hr />
        <p className="text-md md:text-lg mt-8">
          Run the diagnostic script on the affected machine to gather a
          comprehensive snapshot of its network state — including interface
          configuration, routing tables, DNS settings, VPN status, and recent
          system logs. Attaching the resulting archive to your support thread,
          along with a{" "}
          <Link
          ...
          from the relevant client or gateway, can help the Firezone team more
          quickly diagnose your issue.
        </p>
        <div className="mt-8">
          <TabsGroup>
            <TabsItem title="macOS" icon={FaApple}>
              ...
              <div className="hljs rounded text-sm flex items-center pr-2">
                <pre className="flex-1 p-4 overflow-x-auto">
                  <code>{MACOS_DIAG_CMD}</code>
                </pre>
                <CopyButton value={MACOS_DIAG_CMD} />
              </div>
              <p className="mt-2 text-xs">
                <Link
                  href={"/diag/macos.sh" as Route}
                  className="text-accent-500 hover:underline"
                  target="_blank"
