---
title: "Microsoft Entra ID Directory Sync"
description:
  "Automatically synchronize users and groups from Microsoft Entra ID to
  Firezone."
---
...
<Alert color="info">
  <div>
    ...
    For large directories, consider using **Assigned groups only** to limit
    which groups are synced.

    Users originally created from this directory may also be removed if they no
    longer belong to any synced groups.
...
Firezone removes deleted Entra users on the next sync. It also filters out users
...
- **Defense in depth:** Firezone also re-checks `accountEnabled` after receiving
  user payloads, including batched lookups for directly assigned users.
