...
Firezone supports directory sync from Google Workspace, Microsoft Entra ID, and
Okta. To enable directory sync for any of those identity providers please follow
the specific guide for [Google Workspace](/kb/directory-sync/google),
[Microsoft Entra](/kb/directory-sync/entra), or [Okta](/kb/directory-sync/okta).
Once the sync is activated, users, groups, and organizational units will be
synced from your identity provider every few minutes.
