...
You'll first need one more DNS resolvers to use. See the table below for
...
<Alert color="warning">
  Each provider varies greatly in which domains they block which will affect
  their effectiveness depending on your security requirements. We recommend
  testing a few providers to see which one works best for you.
</Alert>
...
<Alert color="info">
  Firezone currently supports upstream resolvers available over UDP/53 only. See
  [this GitHub issue](https://github.com/firezone/firezone/issues/4668) if you
  need support for DNS-over-HTTPS (DoH) or DNS-over-TLS (DoT) resolvers.
</Alert>

## Step 3: Open the Client and sign in

1. Open the Firezone Client on your device.
1. Sign in with your Firezone account credentials.

## Step 4: Verify

You can verify that DNS is being filtered by querying a known malicious domain
and checking if you receive an answer of `0.0.0.0` or `NXDOMAIN`.
