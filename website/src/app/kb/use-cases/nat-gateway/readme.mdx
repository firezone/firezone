...
This is commonly used when you need your team's traffic to appear to come from a
single, static IP address to use services that require source traffic to come
from specified IP whitelists.

After completing this guide, your team's traffic will be routed to a Firezone
Gateway and then out to the internet using its public IP address like the
following diagram illustrates:

<Alert color="info">
  Looking for a high-availability version of this guide using multiple Gateways?
  Read our guide on [scaling access to a VPC](/kb/use-cases/scale-vpc-access).
</Alert>
...
1. Add Resource(s) for each service that requires an IP whitelist. For example,
