...
genearted automatically during sign up. This is used in the URL for your
...
Firezone. Actors can be members of one or more [Groups](/kb/deploy/groups) which
can be assigned one or more [Policies](/kb/deploy/policies) to grant access to
[Resources](/kb/deploy/resources).
...
and can be used in Policies to give all users in that Group access to a
Resource. Users and groups can be automatically synced from your Identity
Provider (Google Workspace only) to ensure only active users and groups maintain
...
manage access to internal or external applications and optionally be configured
to match all subdomains as well. CIDR-based Resources can be used to manage
access for an entire subnets, similar to a traditional VPN.
