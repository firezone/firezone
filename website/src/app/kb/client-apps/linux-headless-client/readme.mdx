---
title: "Install the Linux Headless CLI"
...
The Linux Headless Client is intended for environments without a graphical
desktop session. It supports two authentication modes:
...
- **User tokens via browser-based sign-in** -- short-lived tokens obtained by
  ...
  human user to authenticate the Client on a server they manage without
  installing a GUI. See
...
<Alert color="info">
  ...
  provider through a graphical interface, see the [Linux GUI
...
- A Firezone account, plus admin access to create a service account if you
  intend to authenticate with a service account token
...
[service account](/kb/authenticate/service-accounts) documentation, then start
the Client with the token in the environment:
