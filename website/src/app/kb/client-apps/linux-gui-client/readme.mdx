...
sudo wget https://artifacts.firezone.dev/apt/key.gpg -O /etc/apt/keyrings/firezone.gpg
...
To finish the setup, reboot your machine.
This is necessary to fully reflect group membership changes.
The GUI client cannot function until the current user is in the `firezone-client` group.
