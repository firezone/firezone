...
5. `sudo systemctl enable firezone-tunnel-service.service` (See
...
The Tunnel service (`firezone-tunnel-service.service`) also logs to stdout which is
captured by systemd and sent to journald. To view the logs of the Tunnel service,
use:

```bash
journalctl --pager-end --follow --unit firezone-tunnel-service.service
```
...
```bash
sudo systemctl restart firezone-tunnel-service
```
