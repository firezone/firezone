...
Firezone uses version bands to express compatibility between components. Across
all components, anything in the 1.X band is guaranteed to be compatible with
each other.

Furthermore, Gateways are guaranteed to be compatible with Clients of the
**current** and **previous** version band. For example, Gateway version 1.4.X is
compatible with all Client versions 1.3.X and 1.4.X but not with Clients from
the 1.2.X band.
...
compatible Gateway for each Client according to the compatibility band described
above.

To support Clients across multiple version bands, you can deploy Gateways of
...
In order to safely upgrade your Gateways to a new version band, you'll want to
ensure that all of your Clients have upgraded **at least** to the prior band.
