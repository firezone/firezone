...
The universal OIDC connector **requires** PKCE (Proof Key for Code Exchange) to
be enabled on the connector. PKCE is a security feature that helps prevent
certain types of attacks and is recommended for added security.
...
Users must be created in Firezone before they can authenticate with the
universal OIDC connector. You can [create users](/kb/deploy/users) manually in
the admin portal or use the [REST API](/kb/reference/rest-api) to create users
programmatically.

Groups used for access control must also be created in Firezone before they can
be used with users associated with the universal OIDC connector. You can
