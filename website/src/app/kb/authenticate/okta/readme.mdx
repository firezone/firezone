...
authenticating users and syncing users and groups to Firezone. Use this guide if
you're looking to setup SSO with Okta for your Firezone Enterprise account and
want automatic user / group syncing between Microsoft Entra and Firezone.

<Alert color="info">
  If you're just looking to authenticate users against Okta **without**
  automatic user / group sync, use our [universal OIDC
  connector](/kb/authenticate/oidc) instead, available on all plans.
</Alert>

## Overview

The Firezone Okta connector integrates with Microsoft's identity APIs to support
user authentication and user / group sync.
...
Okta. [Read more](/kb/authenticate/user-group-sync) about how sync works.
...
To setup the Okta connector, follow the steps below.
...
authenticate users and sync users and groups with your Okta account:
...
- **Authorized redirect URIs**: Click **ADD URI**, and enter the two redirect
  URIs shown on the Okta identity provider setup screen in your Firezone admin
  dashboard
  (`Settings -> Identity Providers -> Add Identity Provider -> Okta -> Configure`).
...
Finally, click **Connect Identity Provider** and click **Allow** when Okta
prompts you.
...
done! Your Okta connector is now successfully configured. Users in your
organization can now authenticate to Firezone using this connector and user /
group sync will occur automatically.
