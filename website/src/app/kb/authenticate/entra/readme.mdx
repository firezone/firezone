...
Firezone integrates with Microsoft Entra using a custom connector that supports
both authenticating users and syncing users and groups to Firezone. Use this
guide if you're looking to setup SSO with Microsoft Entra for your Firezone
Enterprise account and want automatic user / group syncing between Microsoft
Entra and Firezone.

<Alert
  color="info"
  html={`
    If you're just looking to authenticate users against Microsoft Entra
    <strong>without</strong> automatic user / group sync, use our
    <a href="/kb/authenticate/oidc" class="text-accent-500 hover:underline">
      universal OIDC connector
    </a> instead, available on all plans.
  `}
/>

## Overview

The Firezone Microsoft Entra connector integrates with Microsoft's identity APIs
...
Microsoft Entra. [Read more](/kb/authenticate/user-group-sync) about how sync
works.

## Setup

Setting up the Microsoft Entra connector is similar to the process of setting up
a universal OIDC connector. The main difference is the addition of a few extra
read-only scopes needed to enable sync.

To setup the Microsoft Entra connector, follow the steps below.

### Step 1: Create a new project in Azure
...
in your Azure account and fill in the following fields:
...
authenticate users and sync users and groups with your Microsoft Entra account:
...
- **Authorized redirect URIs**: Click **ADD URI**, and enter the two redirect
  URIs shown on the Microsoft Entra identity provider setup screen in your
  Firezone admin dashboard
  (`Settings -> Identity Providers -> Add Identity Provider -> Microsoft Entra -> Configure`).
...
done! Your Microsoft Entra connector is now successfully configured. Users in
