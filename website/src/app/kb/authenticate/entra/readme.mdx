...
using a custom connector that supports both authenticating users and syncing
users and groups to Firezone. Use this guide if you're looking to setup SSO with
Microsoft Entra ID for your Firezone Enterprise account and need automatic user
/ group syncing between Entra ID and Firezone.
...
APIs to support user authentication and user / group sync.
...
Entra ID. [Read more](/kb/authenticate/user-group-sync) about how sync works.
...
done! Your Entra ID provider is now successfully configured. Users in your
