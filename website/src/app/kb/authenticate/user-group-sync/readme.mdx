...
To preserve audit log trails, Firezone <strong>never</strong> automatically
deletes synced users or groups. Admins must manually delete users in the admin
portal in order to fully remove them from the system.

### Deleting or suspending users

When a user is deleted or suspended in your Google Workspace account, Firezone
will automatically clear all active client and admin portal web sessions for
that user upon the next user/group sync. The user will be **signed out of all
clients** and forced to reauthenticate.
...
Workspace. User membership is determined by its immediate parent.
