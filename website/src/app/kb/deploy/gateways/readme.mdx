...
| `FIREZONE_ID`           |               | Unique identifier for this Gateway. We recommend using a 64-character hex string, e.g. `$(head -c 32 /dev/urandom \| sha256sum \| cut -d' ' -f1)`.                                                                                                             |
| `FIREZONE_NAME`         |               | Friendly name for this Gateway to display in the admin portal.                                                                                                                                                                                                 |
| `FIREZONE_NO_TELEMETRY` | `false`       | Set to `true` to disable the Sentry.io crash-reporting agent.                                                                                                                                                                                                  |
...
| `RUST_LOG`              | `info`        | Log level for the Gateway. Common values: `error`, `warn`, `info`, `debug`, `trace`. For flow logs, set to `flow_logs=trace`. Read more [here](https://docs.rs/env_logger/latest/env_logger/#enabling-logging).                                                |
