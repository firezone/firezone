...
| AWS      | [Network ACLs](https://docs.aws.amazon.com/vpc/latest/userguide/vpc-network-acls.html)                              | Stateless | By default, AWS ACLs allow all inbound. If you've modified these, be that `UDP 1024-65536` is allowed from source `0.0.0.0/0` so your Clients can connect directly. If you wish to allow **only** Relayed connections, use the source IPs in [relay-ips.json](/relay-ips.json). |
