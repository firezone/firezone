...
title: Clients
description:
  "Distribute Firezone Clients to your team across macOS, Windows, Linux, iOS,
  Android, and ChromeOS. See the client distribution guide."
---
...
are all read from the `HKEY_LOCAL_MACHINE\Software\Policies\Firezone` registry
key (machine-scope, i.e. "Computer Configuration") by the Firezone Tunnel
service when the Client connects.
...
1. Go through the wizard, you can select from all available configuration
   settings for Firezone on the 2nd step. The Firezone settings appear under
   "Computer Configuration"; assign the resulting profile to **device** groups.

<Alert color="info">
  **Upgrading from an earlier Windows Client:** previous versions read these
  policies from the per-user `HKEY_CURRENT_USER` hive. The first time a user
  launches an upgraded Client, the Firezone Tunnel service automatically copies
  any existing values into `HKEY_LOCAL_MACHINE` and removes the old per-user
  key, so settings already deployed to a device keep working without any user
  action. To keep managing the policies after upgrading, re-import the ADMX
  template above and recreate the profile as a Computer Configuration policy
  assigned to device groups; otherwise Intune keeps writing to the per-user
  hive, which the upgraded Client no longer reads.
</Alert>

</TabsItem>

{/* TODO <TabsItem title="iOS" icon="apple"></TabsItem> */}
...
ChromeOS platforms. Headless Clients support two ways to authenticate:

- A long-lived [Service Account](/kb/authenticate/service-accounts) token --
  best for deploying the Client on servers, IoT devices, and other unattended
  systems.
- A short-lived user token obtained via browser-based sign-in -- available on
  the Linux and Windows headless Clients via the `sign-in` subcommand, suitable
  when a human user wants to authenticate the Client without using a GUI.
