...
when you're signed out, Resources will be inaccessible and your system will use its
normal DNS and Internet behavior.
...
The tunnel is now stopped. If you were signed in, the tunnel will restart automatically
...
installer package above, quit the GUI, and install the new MSI over the existing version.
...
If Firezone crashes and does not revert control of the system's DNS, you
can revert it manually with this command:
...
1. When UAC asks "Do you want to allow this app to make changes to your device?" click `Yes`
1. Enter the above command and [Check if Firezone is controlling DNS](#check-if-firezone-is-controlling-dns)
