...
To check that requests are going through Firezone, check if `curl` reports a remote IP
in Firezone's range when you connect to a resource:
...
TODO (DNS control, reverting DNS)
