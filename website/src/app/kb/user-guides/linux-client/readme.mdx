...
server-to-server connectivity where a user isn't present to authenticate with
...
The Linux headless Client can be downloaded from our
[main repository's releases page](https://github.com/firezone/firezone/releases),
or using one of the direct links below:
...
Headless Clients require a service account token to authenticate to Firezone. If
you don't already have a token generated, follow the instructions in the
...
<Alert color="info">
  `sudo` is required to allow control of the system's DNS settings.
</Alert>
