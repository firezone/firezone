...
Components in Firezone can be broadly divided into two categories:

1. **Firezone-managed**: Components that we host and operate for you.
1. **Self-hosted**: Components you host and/or operate yourself.

<Image
  src="/images/kb/architecture/core-components/component-diagram.png"
...
Firezone for your organization. It provides a single pane of glass for
configuring Resources, Policies, and more.

The portal is also responsible for authenticating users, managing your
organization's billing settings, delivering email notifications, and more.
...
lifetime that varies depending on the type of connection. See
[security](/kb/architecture/security) for more information regarding token
lifetimes.
...
<Alert color="warning">
  ...
  Relays do not posess the ability to decrypt or otherwise tamper with the data
  being relayed.
</Alert>
...
Gateways no persistent storage and only a handful of environment variables to be
set in order to function. For more information on deploying Gateways, see the
