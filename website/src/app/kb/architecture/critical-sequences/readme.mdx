...
However, the OIDC flow redirects the user to the identity provder for
...
1. The Client receives this callback URL and validates the `state` parameter
   matches what it originally sent.
...
Firezone employs a unique, granular approach to split-horizon DNS to ensure
traffic intended **only** for DNS-based Resources is routed through Firezone,
leaving other traffic untouched -- even when resolved IP addresses overlap.
...
> nslookup github.com Server: 100.100.111.1 Address: 100.100.111.1#53

Non-authoritative answer: Name: github.com Address: 100.96.0.1
