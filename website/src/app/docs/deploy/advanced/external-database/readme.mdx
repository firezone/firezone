...
:::warning Configuring Firezone to use an external database can be complicated
and error-prone. We recommend using the bundled Postgres for Omnibus-based
...
:::note The official `postgres` docker image can be configured by setting
