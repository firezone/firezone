import { TabsItem, TabsGroup, Alert } from "@/components/Flowbite"
...
<Alert color="warning">
Replacing the `DATABASE_ENCRYPTION_KEY` will render all encrypted
data in the database useless. This **will** break your Firezone install unless
you are starting with an empty database. You have been warned.
</Alert>

<Alert color="danger">
Replacing `GUARDIAN_SECRET_KEY`, `SECRET_KEY_BASE`,
`LIVE_VIEW_SIGNING_SALT`, `COOKIE_SIGNING_SALT`, and `COOKIE_ENCRYPTION_SALT`
...
<Alert color="warning">
Replacing the WireGuard private key will render all existing device
configs invalid. Only do so if you're prepared to also regenerate device configs
