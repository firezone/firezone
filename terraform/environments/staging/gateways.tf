...
module "gateways" {
  ...
  artifact_url = "https://storage.googleapis.com/firezone-staging-artifacts/firezone-gateway"
...
resource "google_compute_firewall" "gateways-ssh-ipv6" {
  count = var.gateway_token != null ? 1 : 0

  project = module.google-cloud-project.project.project_id

  name    = "gateways-ssh-ipv6"
  network = module.google-cloud-vpc.id
