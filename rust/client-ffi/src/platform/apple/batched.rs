//! Batched TUN I/O using Apple's `recvmsg_x` / `sendmsg_x` (see [`super::sys`]).
//!
//! These mirror [`tun::unix::tun_send`] / [`tun::unix::tun_recv`] but exchange a
            ...
            loop {
                ...
                while offset < batch.len() {
                    ...
                    match result {
                        ...
                        Ok(n) => {
                            batch_count
                                .record(n as u64, &[otel::attr::network_io_direction_transmit()]);
                            offset += n;
                        }
                        Err(e) => {
    ...
    tokio::runtime::Builder::new_current_thread()
        ...
        .block_on(async move {
            ...
            loop {
                ...
                batch_count.record(n as u64, &[otel::attr::network_io_direction_receive()]);
...
fn drop_attributes(e: &io::Error) -> [opentelemetry::KeyValue; 3] {
    [
        ...
        opentelemetry::KeyValue::new("error.code", e.raw_os_error().unwrap_or_default() as i64),
    ]
}
