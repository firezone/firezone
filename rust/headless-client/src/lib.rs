...
pub fn run_headless() -> Result<()> {
    // We try to get the token immediately here, note:
    // - We can't use `clap` because it wants to make the token a positional CLI arg
    // - We remove it from the environment so that child processes / other bits of code can't take it by accident
    //
    // Also, systemd recommends against passing secrets through env vars:
...
fn read_token_file(cli: &Cli) -> Result<Option<SecretString>> {
    ...
    if std::fs::metadata(&path).is_err() {
        tracing::info!("No token in env var or on disk");
        return Ok(None);
    }
