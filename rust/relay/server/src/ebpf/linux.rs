...
fn set_interface_ipv4_address(ebpf: &mut aya::Ebpf, addr: Ipv4Addr) -> Result<()> {
    ...
    tracing::info!("Set eBPF interface IPv4 address: {addr}");
    Ok(())
}
...
fn set_interface_ipv6_address(ebpf: &mut aya::Ebpf, addr: Ipv6Addr) -> Result<()> {
    ...
    tracing::info!("Set eBPF interface IPv6 address: {addr}");
    Ok(())
}
    ...
    T: Pod + Clone,
{
    ...
    tracing::debug!("Set per-CPU map {map_name} with value on {num_cpus} CPUs");
    Ok(())
}
