...
fn try_handle_turn(ctx: &XdpContext) -> Result<u32, Error> {
    // SAFETY: This is the only instance of `EthHdr` in this scope.
...
fn try_handle_turn_ipv4(ctx: &XdpContext) -> Result<(), Error> {
    // SAFETY: This is the only instance of `Ipv4Hdr` in this scope.
    ...
    // SAFETY: This is the only instance of `UdpHdr` in this scope.
    let udp = unsafe { ref_mut_at::<UdpHdr>(ctx, EthHdr::LEN + Ipv4Hdr::LEN)? };
    let udp_payload_len = udp.len();
...
fn try_handle_ipv4_udp_to_channel_data(ctx: &XdpContext) -> Result<(), Error> {
    ...
    if let Some(client_and_channel) = unsafe { UDP_TO_CHAN_44.get(&key) } {
        handle_ipv4_to_ipv4_udp_to_channel(ctx, client_and_channel)?;
        return Ok(());
    }

    // SAFETY: We only write to these using a single thread in userspace.
    if let Some(client_and_channel) = unsafe { UDP_TO_CHAN_46.get(&key) } {
        handle_ipv4_to_ipv6_udp_to_channel(ctx, client_and_channel)?;
        return Ok(());
    }
...
#[inline(always)]
fn handle_ipv4_to_ipv4_udp_to_channel(
    ctx: &XdpContext,
    client_and_channel: &ClientAndChannelV4,
) -> Result<(), Error> {
    const NET_EXPANSION: i32 = -(CdHdr::LEN as i32);

    // SAFETY: The BPF helpers check bounds for us
    let ret = unsafe { bpf_xdp_adjust_head(ctx.ctx, NET_EXPANSION) };
    if ret < 0 {
    ...
    // SAFETY: We copy the fields we need from the old packet so that we don't need to
    // read from the old struct after we've modified the packet.
    // SAFETY: After packet adjustment, old_data_offset points to valid packet data.
    ...
    // SAFETY: We assign these fields from the values we copied above.
    let eth = unsafe { ref_mut_at::<EthHdr>(ctx, 0)? };
    ...
    // SAFETY: Writing to the new packet location after expansion.
    // The ref_mut_at helper performs bounds checks before creating the reference.
    // SAFETY: XDP context provides valid packet memory. Bounds checked by ref_mut_at.
    let udp = unsafe { ref_mut_at::<UdpHdr>(ctx, EthHdr::LEN + Ipv4Hdr::LEN)? };
...
#[inline(always)]
fn handle_ipv4_to_ipv6_udp_to_channel(
    ctx: &XdpContext,
    ...
    // Expand FIRST
    // SAFETY: bpf_xdp_adjust_head is a kernel helper that safely adjusts packet headroom.
    let ret = unsafe { bpf_xdp_adjust_head(ctx.ctx, NET_EXPANSION) };
    if ret < 0 {
        return Err(Error::XdpAdjustHeadFailed(ret));
    }
    ...
    // SAFETY: Operation is safe within eBPF context.
    ...
    // 4. Channel data header
    // SAFETY: XDP context provides valid packet memory. Bounds checked by ref_mut_at.
    let cd_ptr = unsafe { ref_mut_at::<CdHdr>(ctx, EthHdr::LEN + Ipv6Hdr::LEN + UdpHdr::LEN)? };
    (*cd_ptr) = CdHdr {
...
fn try_handle_ipv4_channel_data_to_udp(ctx: &XdpContext) -> Result<(), Error> {
    ...
    if let Some(port_and_peer) = unsafe { CHAN_TO_UDP_44.get(&key) } {
        ...
        handle_ipv4_to_ipv4_channel_to_udp(ctx, port_and_peer)?;
        return Ok(());
    }

    // SAFETY: We only write to these using a single thread in userspace.
    if let Some(port_and_peer) = unsafe { CHAN_TO_UDP_46.get(&key) } {
        handle_ipv4_to_ipv6_channel_to_udp(ctx, port_and_peer)?;
        return Ok(());
    }
...
#[inline(always)]
fn handle_ipv4_to_ipv4_channel_to_udp(
    ctx: &XdpContext,
    ...
    // Read channel data header before we lose it
    // SAFETY: XDP context provides valid packet memory. Bounds checked by ref_mut_at.
    ...
    // 1. Ethernet header
    //
    // SAFETY: After packet shrinking, the offset is valid within adjusted bounds.
    ...
    (*udp_ptr).set_source(new_udp_src);
    (*udp_ptr).set_dest(new_udp_dst);
    (*udp_ptr).set_len(new_udp_len);
    ...
    if old_udp_check == 0 || !crate::config::udp_checksum_enabled() {
        ...
        if check == 0 {
            (*udp_ptr).set_check(0xFFFF); // Special case for zero checksum - write 0xFFFF to the wire
        } else {
            (*udp_ptr).set_check(check);
        }
    }

    // Now shrink the packet by removing the first 4 bytes
    // SAFETY: bpf_xdp_adjust_head is a kernel helper that safely adjusts packet headroom.
    let ret = unsafe { bpf_xdp_adjust_head(ctx.ctx, NET_SHRINK) };
    if ret < 0 {
        return Err(Error::XdpAdjustHeadFailed(ret));
    }
...
#[inline(always)]
fn handle_ipv4_to_ipv6_channel_to_udp(
    ctx: &XdpContext,
    port_and_peer: &PortAndPeerV6,
) -> Result<(), Error> {
    // Expand FIRST - 20 - 40 + 4 = -16 bytes
...
fn try_handle_ipv6_udp_to_channel_data(ctx: &XdpContext) -> Result<(), Error> {
    ...
    // SAFETY: We only write to these using a single thread in userspace.
    // Check if this is IPv6 to IPv6 relay
    if let Some(client_and_channel) = unsafe { UDP_TO_CHAN_66.get(&key) } {
        handle_ipv6_to_ipv6_udp_to_channel(ctx, client_and_channel)?;
        return Ok(());
    }

    // // Check if this is IPv6 to IPv4 relay (peer is IPv6, client is IPv4)
    if let Some(client_and_channel) = unsafe { UDP_TO_CHAN_64.get(&key) } {
        handle_ipv6_to_ipv4_udp_to_channel(ctx, client_and_channel)?;
        return Ok(());
    }
...
#[inline(always)]
fn handle_ipv6_to_ipv6_udp_to_channel(
    ctx: &XdpContext,
    client_and_channel: &ClientAndChannelV6,
) -> Result<(), Error> {
    const NET_EXPANSION: i32 = -(CdHdr::LEN as i32); // Expand by 4 bytes for channel data header

    // Expand packet by 4 bytes for channel data header FIRST
    // SAFETY: bpf_xdp_adjust_head is a kernel helper that safely adjusts packet headroom.
    let ret = unsafe { bpf_xdp_adjust_head(ctx.ctx, NET_EXPANSION) };
    if ret < 0 {
        return Err(Error::XdpAdjustHeadFailed(ret));
...
#[inline(always)]
fn handle_ipv6_to_ipv4_udp_to_channel(
    ctx: &XdpContext,
    client_and_channel: &ClientAndChannelV4,
) -> Result<(), Error> {
    ...
    // Read current headers before adjustment
    // SAFETY: XDP context provides valid packet memory. Bounds checked by ref_mut_at.
    let old_eth = unsafe { ref_mut_at::<EthHdr>(ctx, 0)? };
    let old_eth_src = old_eth.src_addr;
    let old_eth_dst = old_eth.dst_addr;
    ...
    // Now shrink the packet by removing the first 16 bytes
    // SAFETY: bpf_xdp_adjust_head is a kernel helper that safely adjusts packet headroom.
    let ret = unsafe { bpf_xdp_adjust_head(ctx.ctx, NET_SHRINK) };
    if ret < 0 {
        return Err(Error::XdpAdjustHeadFailed(ret));
...
fn try_handle_ipv6_channel_data_to_udp(ctx: &XdpContext) -> Result<(), Error> {
    // SAFETY: This is the only instance of `Ipv6Hdr` in this scope.
    let ipv6 = unsafe { ref_mut_at::<Ipv6Hdr>(ctx, EthHdr::LEN)? };

    // SAFETY: This is the only instance of `UdpHdr` in this scope.
    let udp = unsafe { ref_mut_at::<UdpHdr>(ctx, EthHdr::LEN + Ipv6Hdr::LEN)? };

    // SAFETY: This is the only instance of `CdHdr` in this scope.
    ...
    if let Some(port_and_peer) = unsafe { CHAN_TO_UDP_66.get(&key) } {
        handle_ipv6_to_ipv6_channel_to_udp(ctx, port_and_peer)?;
        return Ok(());
    }

    // SAFETY: We only write to these using a single thread in userspace.
    if let Some(port_and_peer) = unsafe { CHAN_TO_UDP_64.get(&key) } {
        handle_ipv6_to_ipv4_channel_to_udp(ctx, port_and_peer)?;
        return Ok(());
    }
...
#[inline(always)]
fn handle_ipv6_to_ipv6_channel_to_udp(
    ctx: &XdpContext,
    port_and_peer: &PortAndPeerV6,
) -> Result<(), Error> {
    const NET_SHRINK: i32 = CdHdr::LEN as i32; // Shrink by 4 bytes for channel data header
...
#[inline(always)]
fn handle_ipv6_to_ipv4_channel_to_udp(
    ctx: &XdpContext,
    port_and_peer: &PortAndPeerV4,
) -> Result<(), Error> {
    ...
    // Re-parse headers to get current values before adjustment
    // SAFETY: XDP context provides valid packet memory. Bounds checked by ref_mut_at.
    let old_eth = unsafe { ref_mut_at::<EthHdr>(ctx, 0)? };
    let old_eth_src = old_eth.src_addr;
    let old_eth_dst = old_eth.dst_addr;
    ...
    // Now shrink the packet by removing the first 24 bytes
    // SAFETY: bpf_xdp_adjust_head is a kernel helper that safely adjusts packet headroom.
    let ret = unsafe { bpf_xdp_adjust_head(ctx.ctx, NET_SHRINK) };
    if ret < 0 {
        return Err(Error::XdpAdjustHeadFailed(ret));
