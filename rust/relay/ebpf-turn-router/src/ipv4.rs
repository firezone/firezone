use crate::{checksum::ChecksumUpdate, slice_mut_at::slice_mut_at, Error};
...
impl<'a> Ipv4<'a> {
    pub fn parse(ctx: &'a XdpContext) -> Result<Self, Error> {
        ...
        let ipv4_slice = Ipv4HeaderSlice::from_slice(slice_mut).map_err(|_| Error::Ipv4Header)?;
        ...
        Ok(Self {
            ...
            slice_mut: Ipv4HeaderSliceMut::from_slice(slice_mut).map_err(|_| Error::Ipv4Header)?,
        })
    }
    ...
    pub fn update(mut self, new_src: [u8; 4], new_dst: [u8; 4], new_len: u16) -> ChecksumUpdate {
        ...
        debug!(
            self.ctx,
            "IPv4 header update: src {:i} -> {:i}; dst {:i} -> {:i}",
            self.src,
            new_src,
            self.dst,
            new_dst,
