...
fn revert_at_paths(paths: &ResolvPaths) -> Result<(), Error> {
    ...
    std::fs::remove_file(&paths.backup).map_err(Error::Revert)?;
    Ok(())
}
...
async fn configure_at_paths(dns_config: &[IpAddr], paths: &ResolvPaths) -> Result<(), Error> {
    ...
    match backup_file.write(|f| f.write_all(text.as_bytes())) {
        ...
        {
            tracing::info!(?paths.backup, "Backup path already exists, won't overwrite");
        }
...
"
    ...
    async fn resolv_conf_happy_path() -> Result<()> {
        ...
        assert!(!tokio::fs::try_exists(&paths.backup).await?);
    ...
    async fn resolv_conf_no_sentinels() -> Result<()> {
        ...
        check_resolv_conf(&paths.resolv, &[GOOGLE_DNS.into()])?;
        ensure!(Path::try_exists(&paths.backup)? == false);
