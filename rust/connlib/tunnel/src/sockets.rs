...
impl ThreadedUdpSocket {
    ...
    fn new(sf: &dyn SocketFactory<UdpSocket>, mut addr: SocketAddr) -> io::Result<Self> {
        let socket = sf(&addr)?;
        addr.set_port(socket.local_port());
        ...
        for n in 0..4 {
            std::thread::Builder::new()
                .name(
                    match addr {
                        SocketAddr::V4(_) => format!("UDPv4 {}/4", n + 1),
                        SocketAddr::V6(_) => format!("UDPv6 {}/4", n + 1),
                    },
                )
                .spawn({
