...
use connlib_shared::{CallbackErrorFacade, Callbacks, Result};
use std::sync::Arc;
use windows::Win32::{
    Foundation::NO_ERROR,
    Foundation::BOOLEAN,
    NetworkManagement::IpHelper::{
        AddIPAddress, MIB_IPFORWARD_ROW2,
        CreateIpForwardEntry2,
        GetUnicastIpAddressTable, MIB_UNICASTIPADDRESS_TABLE, MIB_UNICASTIPADDRESS_ROW, DeleteUnicastIpAddressEntry, FreeMibTable,
        GetIpInterfaceEntry,SetIpInterfaceEntry, MIB_IPINTERFACE_ROW
    },
    Networking::WinSock::{AF_INET, RouterDiscoveryDisabled, SOCKADDR_INET,MIB_IPPROTO_NETMGMT, htonl},
};
use crate::InterfaceConfig;
use connlib_shared::Error;
use connlib_shared::Error::{
    OnSetInterfaceConfigFailed,
    OnAddRouteFailed
};
...
pub struct IfaceDevice {
    adapter_index: u32,    
    reader_session: Arc<wintun::Session>,
    writer_session: Arc<wintun::Session>,
    mtu: u32,
    ipv4_address:u32,
    ip_context: u32,
    ip_instance: u32
...
impl Drop for IfaceDevice {
    fn drop(&mut self) {
        ...
        let _= self.reader_session.shutdown();
    }
}

impl IfaceDevice {
    fn write(&self, buf: &[u8]) -> usize {
        let mut packet = self.writer_session.allocate_send_packet(buf.len() as u16).unwrap();
        packet.bytes_mut().copy_from_slice(buf.as_ref());

        self.writer_session.send_packet(packet);
        buf.len()
    ...
    fn set_iface_ipv4(&mut self, config: &InterfaceConfig) -> Result<()>{
        ...
        unsafe {
            ...
            let result = AddIPAddress( htonl(config.ipv4.into()),
                                    IPV4_NETMASK_32,
                                   self.adapter_index,
                                &mut self.ip_context,
                               &mut self.ip_instance);
            if result != NO_ERROR.0 {
                return Err(OnSetInterfaceConfigFailed(format!("AddIPAddress failed with error code: {}", result)));
            }
            Ok(())
        }
    }
    ...
    pub async fn read<'a>(&self, dst: &'a mut [u8]) -> Result<&'a mut [u8]> {
        let reader_session = self.reader_session.clone();
 
        let result = tokio::task::spawn_blocking(move || {
            reader_session.receive_blocking()
        }).await;
        match result.unwrap() {
            Ok(packet) => {
            ...
            Err(err) => {
                Err(Error::IfaceRead(std::io::Error::new(std::io::ErrorKind::Other, err)))
            }
        }
    }
    ...
    pub async fn add_route(
        ...
        match route {
            IpNetwork::V4(ipnet) => {
                ...
                sockaddr_inet.Ipv4.sin_addr.S_un.S_addr = unsafe { htonl(ipnet.network_address().into())};
                ...
                unsafe {
                     CreateIpForwardEntry2(&mut route).map_err(|err|  OnAddRouteFailed(err.to_string()))?
                };
                Ok(())
            }
            IpNetwork::V6(_) => {
                Err(OnAddRouteFailed(format!("Not implemented")))
            }
        }
    }
