...
async fn configure_resolv_conf(dns_config: &[IpAddr]) -> Result<()> {
    ...
    let text = tokio::fs::read_to_string(resolv_path).await.map_err(|_| Error::ReadResolvConf)?;

    // Back up the original resolv.conf
    tokio::fs::write(backup_path, text).await.map_err(|_| Error::WriteResolvConfBackup)?;
...
";
    ...
    fn print_resolv_conf() {
        ...
        for addr in ["100.100.111.1", "100.100.111.2"] {
            new_resolv_conf.nameservers.push(IpAddr::from_str(addr).unwrap().into());
        }
