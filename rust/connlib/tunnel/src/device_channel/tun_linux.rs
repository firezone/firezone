...
async fn configure_resolv_conf(dns_config: &[IpAddr]) -> Result<()> {
    ...
    let text = tokio::fs::read_to_string(resolv_path).await?;
    let mut conf = resolv_conf::Config::parse(&text).expect("Existing resolv.conf should be parsable");

    // Back up the original resolv.conf
    tokio::fs::write("/etc/resolv.conf.firezone-backup", text).await?;

    for addr in dns_config {
        conf.nameservers.push((*addr).into());
    }

    // Write the new one to a temp file
    // Use a random suffix for good luck
    let temp_path = Path::new("/etc/resolv.conf.3GLNT65C");
    let new_text = format!("# Generated by the Firezone client\n{:#?}", conf);
    tokio::fs::write(temp_path, new_text).await?;

    // Rename it into place
    tokio::fs::rename(temp_path, resolv_path).await?;
...
";
    ...
    fn parse_resolv_conf() {
        ...
        config.nameservers.push(resolv_conf::ScopedIp::V4(Ipv4Addr::new(127, 0, 0, 53)));
        ...
        let mut config = resolv_conf::Config::new();
        config.nameservers.push(resolv_conf::ScopedIp::V4(Ipv4Addr::new(9, 9, 9, 9)));

        assert_eq!(parsed, config);

        let parsed = resolv_conf::Config::parse(NETWORK_MANAGER_RESOLV_CONF).unwrap();
        let mut config = resolv_conf::Config::new();
        config.nameservers.push(resolv_conf::ScopedIp::V4(Ipv4Addr::new(192, 168, 1, 1)));
        config.nameservers.push(resolv_conf::ScopedIp::V6(Ipv6Addr::new(0x2001, 0x0db8, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000), Some("eno1".into())));
