...
impl Device {
    ...
    pub(crate) fn poll_read<'b>(
        ...
        tracing::trace!(target: "wire", from = "device", dest = %packet.destination(), src = %packet.source(), bytes = %packet.packet().len());
    ...
    pub fn write(&self, packet: IpPacket<'_>) -> io::Result<usize> {
        tracing::trace!(target: "wire", to = "device", dest = %packet.destination(), src = %packet.source(), bytes = %packet.packet().len());
