...
impl ClientState {
    ...
    fn handle_udp_dns_query(
        ...
        let message = match parse_udp_dns_query(&datagram) {
            Ok(message) => message,
            Err(e) => {
    ...
    fn mangle_udp_dns_query_to_new_upstream_through_tunnel(
        ...
        let query_id = parse_udp_dns_query(&datagram)
...
fn parse_udp_dns_query(datagram: &UdpSlice) -> anyhow::Result<dns_types::Query> {
    ...
    let query = dns_types::Query::parse(datagram.payload())
        .context("Failed to parse UDP payload as DNS query")?;
