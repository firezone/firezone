#!/usr/bin/env python3
"""Photographs the native AppIndicator menu in an isolated headless GNOME session."""
...
def main():
    ...
    parser.add_argument("--submenu", required=True)
    ...
    with tempfile.TemporaryDirectory(prefix="gnome-tray-") as session:
        try:
            capture(
                args.client.resolve(), args.submenu, output, diagnostics, Path(session)
            )
        finally:
            signal.alarm(0)
            ...
            if os.path.ismount(portal_mount):
                subprocess.run(
                    ["fusermount3", "-u", str(portal_mount)], check=True, timeout=10
                )


def capture(client, submenu, output, diagnostics, session):
    ...
    subprocess.run(
        ["dbus-update-activation-environment", *environment], check=True, timeout=10
    )
    ...
    (diagnostics / "packages.txt").write_text(packages)
    ...
    with (
        (diagnostics / "gnome-shell.log").open("w") as shell_log,
        (diagnostics / "client.log").open("w") as client_log,
    ):
        ...
        try:
            wait_for(
                "GNOME Shell",
                lambda: evaluate("!Main.layoutManager._startingUp"),
                shell,
            )
            wait_for(
            ...
            entries = subprocess.check_output(
                ["gresource", "list", str(resource)], text=True, timeout=10
            ).splitlines()
            css_entry = next(
                entry for entry in entries if entry.endswith("/gnome-shell.css")
            )
            css = session / "yaru.css"
            css.write_bytes(
                subprocess.check_output(
                    ["gresource", "extract", str(resource), css_entry], timeout=10
                )
            )
            ...
            evaluate(f"""(() => {{
                ...
                global.firezoneScreenshotSubmenuLabel = {json.dumps(submenu)};
                return true;
            }})()""")
            subprocess.run(
            ...
            wait_for(
                "Wayland display",
                lambda: (session / "runtime/wayland-0").exists(),
                shell,
            )
            ...
            wait_for(
                f"{submenu!r} in the native menu",
                lambda: evaluate("""(() => {
                ...
                const resource = indicator.menu._getMenuItems().find(
                    item => item.label?.text === global.firezoneScreenshotSubmenuLabel);
                if (!resource?.menu) return false;
                resource.menu.open(false);
                global.firezoneScreenshotMenu = indicator.menu;
                global.firezoneScreenshotSubmenu = resource.menu;
                return resource.menu._getMenuItems().some(item => item.label?.text);
            })()"""),
                shell,
                gui,
            )
            (diagnostics / "menu.json").write_text(
                json.dumps(evaluate("global.firezoneScreenshotState"), indent=2) + "\n"
            )
            ...
            (diagnostics / "geometry.json").write_text(json.dumps(bounds) + "\n")
            screenshot(bounds, output)
            ...
            try:
                ...
                screenshot((0, 0, 1920, 1080), diagnostics / "failure-desktop.png")
            ...
            for process in [gui, shell]:
                ...
                try:
                    ...
                    process.kill()
                    process.wait()
            print((diagnostics / "gnome-shell.log").read_text(), flush=True)
            print((diagnostics / "client.log").read_text(), flush=True)
    ...
    while time.monotonic() < deadline:
        for process in processes:
            if process.poll() is not None:
                raise RuntimeError(
                    f"Process exited with {process.returncode} waiting for {description}"
                )
        try:
            if predicate():
