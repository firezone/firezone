...
use tunnel_proto::ResolvedDevice;
    ...
    fn on_client_event(
        ...
        match event {
            ...
            ClientEvent::DeviceDomainQueried { domain } => {
                // The portal answers a name with the pools that admit the device; access
                // is asked for on the first packet, like for a static pool member.
                let result = match portal.resolve_device_domain(&domain) {
                    None => Err(FailReason::NotFound),
                    Some((id, ipv4, ipv6)) => {
                        let pools = ref_state
                            .clients
                            .get(&src)
