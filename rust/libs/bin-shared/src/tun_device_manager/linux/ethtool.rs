...
mod tests {
    ...
    #[test]
    #[ignore = "Needs root and kernel 5.12+"]
    #[expect(clippy::unwrap_used, reason = "It is a test")]
    fn enables_udp_gro_forwarding_on_tun_device() {
        let name = "tun-test-gro";

        let _tun_fd = create_tun(name);

        enable_feature(name, "rx-udp-gro-forwarding").unwrap();

        assert!(feature_is_active(name, "rx-udp-gro-forwarding").unwrap());
    }

    /// Reads back whether the feature is active, via `ETHTOOL_GFEATURES`.
    ///
    /// `ETHTOOL_SFEATURES` reports refused changes via its (discarded) return value,
    /// so a successful `enable_feature` alone does not prove the bit flipped.
    fn feature_is_active(ifname: &str, feature: &str) -> Result<bool> {
        ...
        struct GFeatures {
            cmd: u32,
            size: u32,
            blocks: [GetFeatureBlock; MAX_FEATURE_WORDS],
        }

        #[repr(C)]
        #[derive(Clone, Copy, Default)]
        struct GetFeatureBlock {
            available: u32,
            requested: u32,
            active: u32,
            never_changed: u32,
        ...
        let socket = ethtool_socket()?;
        let num_features = num_features(&socket, ifname)?;
        let bit = feature_bit(&socket, ifname, num_features, feature)?;

        let mut gfeatures = GFeatures {
            cmd: ETHTOOL_GFEATURES,
            size: num_features.div_ceil(32) as u32,
            blocks: [GetFeatureBlock::default(); MAX_FEATURE_WORDS],
        };

        // Safety: The socket FD is open and `gfeatures` is a valid `ethtool` command struct.
        unsafe {
            ioctl::exec(
                socket.as_raw_fd(),
                libc::SIOCETHTOOL,
                &mut ioctl::Request::<ioctl::EthtoolPayload>::new(ifname, &mut gfeatures),
            )?;
        }

        Ok(gfeatures.blocks[bit / 32].active & (1 << (bit % 32)) != 0)
    }

    /// Creates a TUN device that lives for as long as the returned FD.
    fn create_tun(name: &str) -> OwnedFd {
        // Safety: FFI call with valid arguments.
        let fd = unsafe { libc::open(c"/dev/net/tun".as_ptr().cast(), libc::O_RDWR) };
