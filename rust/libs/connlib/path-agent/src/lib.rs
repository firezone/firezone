...
//! handshake (an init or a response). A response is bidirectionally validated,
//! so it seeds a preliminary primary ranked by candidate tier.
//!
...
pub use agent::{
    PRIMARY_KEEPALIVE, PROBE_BUDGET, PROBE_BURST_GAPS, PROBE_INTERVAL, PROBE_TIMEOUT, PathAgent,
