...
/// Generates a packet that respects the given resource filters.
///
/// When `filters` is non-empty and contains ICMP or UDP entries, this generates:
/// - With 80% probability: a packet that matches one of the compatible filters.
/// - With 20% probability: an unconstrained ICMP or UDP packet.
///
/// TCP filters are intentionally ignored here; TCP connections are generated by
/// [`connect_tcp_for_filters`] instead.
#[expect(private_bounds)]
pub(crate) fn icmp_or_udp_packet_for_filters<I, D>(
    client_id: ClientId,
...
{
    let any_packet = prop_oneof![
        icmp_packet(client_id, src.clone(), dst.clone()),
        udp_packet(client_id, src.clone(), dst.clone(), non_dns_ports()),
    ]
    .boxed();

    // Only ICMP and UDP filters are handled here; TCP connections go through `connect_tcp_for_filters`.
    let filters = filters
        .into_iter()
        .filter(|f| !matches!(f, Filter::Tcp(_)))
        .filter(|f| !matches!(f, Filter::Udp(PortRange { port_range_start, port_range_end }) if *port_range_start == 53 && *port_range_end == 53))
        .collect::<Vec<_>>();
    ...
    let matching = sample::select(filters).prop_flat_map(move |filter| match filter {
        Filter::Icmp => icmp_packet(client_id, src.clone(), dst.clone()).boxed(),
...
/// Generates a TCP connection that respects the given resource filters.
///
    ...
    I: Into<IpAddr> + 'static,
{
    ...
    // Only TCP filters are relevant here; ICMP/UDP packets go through `packet_for_filters`.
    let tcp_filters = filters
        .into_iter()
        .filter_map(|f| match f {
            Filter::Tcp(range) => Some(range),
    ...
    let matching = sample::select(tcp_filters)
        .prop_flat_map(
            move |PortRange {
                      port_range_start,
                      port_range_end,
