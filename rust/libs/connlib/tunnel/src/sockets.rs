...
///
/// A batch is one `recv_from`: `quinn-udp` reads up to `BATCH_SIZE` (64 on Apple, 32 on other unix)
...
/// (GRO), `1316 * 64` ~ 84 KB on Apple. Apple - iOS and macOS alike - has no GRO (`recvmsg_x` only
/// batches individual datagrams), so it drains many of these small batches per poll to sustain
/// throughput, which is cheap. Android's GRO batches are large, so it drains a single one per poll to
/// hold the inbound budget down. Other desktop platforms aren't memory-capped, so a moderate limit
