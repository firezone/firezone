...
impl PendingFlows {
    ...
    pub fn on_not_connected_resource(
        ...
        if !sites.has_preferred_gateway(site.id) && has_pending_flows_for_site {
...
mod tests {
    ...
    use connlib_model::{GatewayId, Site, SiteId};
    ...
    fn skips_connection_intent_if_sent_within_last_two_seconds() {
        ...
        pending_flows.on_not_connected_resource(rid, trigger(1), &resources, &sites, now);
        assert_eq!(pending_flows.poll_connection_intents(), Some(rid));

        now += Duration::from_secs(1);

        pending_flows.on_not_connected_resource(rid, trigger(2), &resources, &sites, now);
    ...
    fn buffers_intent_for_resources_in_same_site() {
        ...
        pending_flows.on_not_connected_resource(rid1, trigger(1), &resources, &sites, now);
        assert_eq!(pending_flows.poll_connection_intents(), Some(rid1));
        pending_flows.on_not_connected_resource(rid2, trigger(2), &resources, &sites, now);
        assert_eq!(pending_flows.poll_connection_intents(), None);

        pending_flows.remove(&rid1);

        pending_flows.on_not_connected_resource(rid2, trigger(3), &resources, &sites, now);
    ...
    fn sends_intent_for_same_site_in_parallel_if_already_connected() {
        ...
        sites.set_preferred_gateway(SiteId::from_u128(1), GatewayId::from_u128(1));
