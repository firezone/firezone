...
impl<S> Tracker<S> {
    ...
    pub fn set_enabled(&mut self, enabled: bool) {
        if self.enabled != enabled {
            tracing::info!(enabled, "Flow tracking toggled");
        }
...
mod tests {
    ...
    fn drive_tx(
        ...
        record_decrypted_packet(packet);
        record_peer(ClientId::from_u128(1), Role::Responder);
        record_ingest_token(Some(test_token(authz_id, "responder")));
    }
