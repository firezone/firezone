...
impl ClientState {
    ...
    pub fn handle_client_device_access_authorized(
        ...
        if let Some(grant) = &outbound {
            self.mark_authorized_via(grant.resource_id, cid, flow_logs_ingest_token.clone());
        ...
        for (resource_id, pending_authorization) in pending_authorizations {
            self.mark_authorized_via(resource_id, cid, flow_logs_ingest_token.clone());
    ...
    fn mark_authorized_via(&mut self, resource_id: ResourceId, cid: ClientId, ingest_token: IngestToken) {
        self.authorized_resources
            .entry(resource_id)
            .or_insert_with(AuthorizedOutboundResource::direct)
            .authorize_client(cid, ingest_token);
    }
