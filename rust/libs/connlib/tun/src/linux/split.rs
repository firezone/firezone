...
pub fn split(buf: &[u8], out: &mut Vec<IpPacket>) -> Result<()> {
...
fn copy_single(hdr: &VirtioNetHdr, packet: &[u8]) -> Result<IpPacket> {
    ...
    IpPacket::new(ip_packet_buf, len).context("Failed to parse IP packet")
}
...
fn split_gso(hdr: &VirtioNetHdr, packet: &[u8], out: &mut Vec<IpPacket>) -> Result<()> {
    let gso_size = hdr.gso_size as usize;
    ensure!(gso_size > 0, "gso_size must not be zero");

    let (v4, protocol, ip_hdr_len) = parse_ip_header(packet)?;
    ...
    let l4_hdr_len = match (hdr.gso_type, protocol) {
        (VIRTIO_NET_HDR_GSO_TCPV4, TCP) | (VIRTIO_NET_HDR_GSO_TCPV6, TCP) => {
            ensure!(
        ...
        (VIRTIO_NET_HDR_GSO_UDP_L4, UDP) => 8,
        (gso_type, protocol) => {
    ...
    let (src, dst) = addresses(packet, v4)?;
    ...
    for (index, segment) in payload.chunks(gso_size).enumerate() {
        ...
        match IpPacket::new(ip_packet_buf, len).context("Failed to parse segment") {
...
fn parse_ip_header(packet: &[u8]) -> Result<(bool, u8, usize)> {
    ...
    match packet[0] >> 4 {
        ...
        6 => {
            ...
            Ok((false, packet[6], 40))
        }
        other => bail!("Not an IP packet (version: {other})"),
    }
}

fn addresses(packet: &[u8], v4: bool) -> Result<(IpAddr, IpAddr)> {
    if v4 {
