...
|---|---|
| Status | Draft |
| Scope | `tun` crate, `client-ffi` (Apple platform module) |
...
- Linux/Android batching. Those fds are character devices — socket batch syscalls don't apply; Linux batching means vnet_hdr/GSO and deserves its own design. The new APIs are additive; `tun_send`/`tun_recv` remain for these platforms and as the Apple fallback.
...
### 1. Batched helpers in the `tun` crate (`tun/src/unix.rs`)

Two new functions alongside `tun_send`/`tun_recv`. Shapes chosen so parsing, logging policy and backpressure stay in the shared crate, while the platform module only does syscalls:

```rust
/// Sends batches of packets to the TUN device.
///
/// `write_many` writes as many packets from the slice as it can in one syscall
/// and returns how many were written. Partial writes are resubmitted.
pub fn tun_send_batched<T: AsRawFd>(
    fd: T,
    outbound_rx: mpsc::Receiver<IpPacket>,
    batch_size: usize,
    write_many: impl Fn(RawFd, &[IpPacket]) -> io::Result<usize>,
) -> Result<()>;

/// Receives batches of packets from the TUN device.
///
/// `read_many` fills up to `bufs.len()` buffers in one syscall and writes each
/// packet's length into `lens`, returning the number of packets read.
pub fn tun_recv_batched<T: AsRawFd>(
    fd: T,
    inbound_tx: mpsc::Sender<IpPacket>,
    batch_size: usize,
    read_many: impl Fn(RawFd, &mut [IpPacketBuf], &mut [usize]) -> io::Result<usize>,
) -> Result<()>;
```

Send loop (per-thread current-thread runtime + `AsyncFd`, as today):
...
loop {
    ...
    while offset < batch.len() {
        match fd.async_io(Interest::WRITABLE, |fd| {
            match write_many(fd.as_raw_fd(), &batch[offset..]) {
...
loop {
    ...
    let n = fd.async_io(Interest::READABLE, |fd| {
        read_many(fd.as_raw_fd(), &mut bufs, &mut lens)
...
- `read_many` returning 0 packets is treated as `WouldBlock` by the implementation (see below), so the loop cannot spin.

### 2. Apple implementation (`client-ffi/src/platform/apple/`)

New `sys` submodule owning the private-API surface:
...
```rust
pub const BATCH_SIZE: usize = if cfg!(target_os = "ios") { 25 } else { 100 };
```
