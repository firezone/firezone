...
/// Translate a str0m candidate into the path-agent's representation.
///
...
pub(crate) fn to_path_agent(c: &is::Candidate) -> path_agent::Candidate {
    let kind = match c.kind() {
        CandidateKind::Host => path_agent::CandidateKind::Host,
        CandidateKind::ServerReflexive | CandidateKind::PeerReflexive => {
            path_agent::CandidateKind::ServerReflexive
        }
        CandidateKind::Relayed => path_agent::CandidateKind::Relayed,
    };

    path_agent::Candidate::new(c.addr(), kind)
}
...
mod tests {
    ...
    #[test]
    fn server_reflexive_maps_to_server_reflexive() {
        let c = is::Candidate::server_reflexive(addr(), other_addr(), "udp").unwrap();
        assert_eq!(
            to_path_agent(&c).kind,
            path_agent::CandidateKind::ServerReflexive
        );
    }
    ...
    fn relayed_maps_to_relayed() {
        ...
        assert_eq!(to_path_agent(&c).kind, path_agent::CandidateKind::Relayed);
    }
}
