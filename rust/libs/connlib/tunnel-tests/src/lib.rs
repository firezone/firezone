...
///
/// Shared by both fuzz entry points ([`run_fuzz_case`] and
/// [`run_fuzz_case_structured`]). Mass fuzzing installs only
/// [`PanicOnErrorEvents`] and writes no logs; setting `RUST_LOG` (e.g. when
/// reproducing a saved crash) additionally writes a trace to stderr.
