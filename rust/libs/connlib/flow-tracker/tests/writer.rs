...
/// Guards the writer's disk budget: it divides a byte budget by
/// [`flow_log_writer::MAX_REPORT_BYTES`] to decide how many reports the spool
/// may hold, so a report that outgrows that bound costs more disk than the
/// spool is allowed to use.
///
/// The bound is not obvious from the field types. A maximum-length DNS name is
/// 253 bytes on the wire, 1003 once written as presentation escapes, and 1255
/// after JSON escapes those backslashes again.
#[test]
fn worst_case_report_fits_the_spool_budget() {
    let authz_id = "33333333-3333-3333-3333-333333333333";
    let spool = SpoolObserver::new(authz_id);

    let escaped_label = "\\000".repeat(63);
    let domain = format!(
        "{escaped_label}.{escaped_label}.{escaped_label}.{}",
        "\\000".repeat(61)
    );
    let v6 = "ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff".parse().unwrap();
    // The widest `Debug` rendering `chrono` produces for a `DateTime<Utc>`.
    let timestamp = chrono::Utc
        .timestamp_opt(253_402_300_799, 999_999_999)
        .unwrap();

    // A completed report carries the closing fields on top of the open ones, so
    // it is the larger of the two a flow spools.
    spool.observe(|| {
        flow_tracker::emit(&Record {
            ingest_token: test_token(authz_id, "responder"),
            protocol: FlowProtocol::Tcp,
            inner_src_ip: v6,
            inner_src_port: u16::MAX,
            inner_dst_ip: v6,
            inner_dst_port: u16::MAX,
            domain: Some(domain.parse().unwrap()),
            outers: (0..flow_tracker::MAX_CONTEXTS_PER_FLOW)
                .map(|_| {
                    FlowContext::new(
                        SocketAddr::new(v6, u16::MAX),
                        SocketAddr::new(v6, u16::MAX),
                        timestamp,
                    )
                })
    ...
    let (path, _) = spool.reports().pop().expect("the report is spooled");
    let bytes = std::fs::metadata(&path).unwrap().len();

    assert!(
        bytes <= flow_log_writer::MAX_REPORT_BYTES,
        "worst-case report is {bytes} bytes, over the {} the spool budgets for one",
        flow_log_writer::MAX_REPORT_BYTES
    );
}
