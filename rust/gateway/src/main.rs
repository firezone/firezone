...
use clap::Parser;
use connlib_shared::{
    get_user_agent, keypair, platform::DnsControlMethod, Callbacks, Cidrv4, Cidrv6, LoginUrl,
    StaticSecret,
};
...
async fn run(login: LoginUrl, private_key: StaticSecret) -> Result<Infallible> {
    ...
    let mut interface =
        connlib_shared::interface::InterfaceManager::new(DnsControlMethod::DontControl);
    interface
