...
async fn try_main(cli: Cli, telemetry: &mut Telemetry) -> Result<ExitCode> {
    ...
    let mut tun_device_manager =
        TunDeviceManager::new(cli.tun_interface, ip_packet::MAX_IP_SIZE, cli.tun_threads)
...
struct Cli {
    ...
    /// Name of the TUN interface that will be created for this gateway
