...
async fn try_main(cli: Cli, telemetry: &mut Telemetry) -> Result<()> {
    ...
    let mut tunnel = GatewayTunnel::new(
        ...
        Arc::new(UdpSocketFactory::default()),
        nameservers,
        cli.flow_logs.then(|| PathBuf::from(FLOW_LOGS_DIR)),
    );

    if cli.flow_logs {
        let ingest_url = cli
            .flow_api_url
            .join("/ingestion/flow_logs")
            .context("Failed to construct flow-log ingest URL")?;
        flow_log_uploader::spawn(PathBuf::from(FLOW_LOGS_DIR), ingest_url.to_string());
    }
...
struct Cli {
    ...
    api_url: Url,
    /// Base URL of the flow-log ingest API.
    #[arg(
        long,
        hide = true,
        env = "FIREZONE_FLOW_API_URL",
        default_value = "https://flow-api.firezone.dev"
    )]
    flow_api_url: Url,
...
mod tests {
    ...
    #[test]
    fn builds_ingest_url_from_flow_api_base() {
        let base: Url = "https://flow-api.firezone.dev".parse().unwrap();
        let ingest = base.join("/ingestion/flow_logs").unwrap();
        assert_eq!(
            ingest.as_str(),
