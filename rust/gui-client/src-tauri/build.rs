...
/// Reads the sparse MSIX manifest, derives the resulting kernel-tracked
/// package identity SID, and emits the values as `cargo:rustc-env=` so
/// the constants in `src/lib.rs` are baked into the binary at compile
/// time.
///
...
/// between this build-time computation and the runtime kernel value.
...
///
/// Microsoft's package-publisher-id algorithm uses this exact form; the
/// spec is publicly documented by reverse-engineering the
/// `CryptCATAdminCalcHashFromFileHandle` derivative used by AppX.
...
/// Derives the package SID from the Package Family Name (`{Name}_{publisher_id}`,
/// lowercased). Five 32-bit subauthorities are little-endian-decoded from
/// the first 20 bytes of `SHA256(UTF16LE(lowercased PFN))`.
...
mod tests {
    ...
    /// Microsoft's well-known publisher hash, used as the canonical test
    /// vector. If this regresses, the algorithm is wrong.
    #[test]
    ...
    /// Microsoft.WindowsCalculator's PFN is a stable Win10/Win11 inbox
    /// app with a documented SID, useful for regression-testing the
    /// second hash + little-endian decode.
    #[test]
    fn microsoft_calculator_sid() {
        let sid = package_sid_from_pfn("Microsoft.WindowsCalculator_8wekyb3d8bbwe");
        // The exact value is verified by `Get-AppxPackage Microsoft.WindowsCalculator | % SID`
        // on a Windows host.
    ...
    fn pfn_lowercasing_matters() {
        // SID derivation uses the lowercased PFN; if we forgot to lowercase,
        // the second hash would diverge from the kernel's value.
