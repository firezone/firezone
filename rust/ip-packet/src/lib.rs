...
use etherparse::{
    icmpv6::ParameterProblemHeader, IpFragOffset, Ipv4Dscp, Ipv4Ecn, Ipv4Options, Ipv6Header,
};
use pnet_packet::{
    icmp::{
...
impl<'a> ConvertibleIpv6Packet<'a> {
    ...
    fn consume_to_ipv4(
        ...
        use etherparse::{Icmpv6Header, IpNumber, Ipv4Header};

        let ipv6_packet = self.buf.as_ref();

        let (headers, payload) = etherparse::IpHeaders::from_ipv6_slice(ipv6_packet).ok()?;
        ...
        if ipv6_header.next_header == IpNumber::IPV6_ICMP {
            ...
            let (_ip_header, ip_payload) = self.buf.split_at_mut(Ipv6Header::LEN);
        ...
        self.buf[..40].fill(0);
        let ipv4_header_buf = &mut self.buf[20..];
