...
docker compose exec --env RUST_LOG=info -it client /bin/sh -c 'iperf3 -t 30 -O 1 -u -b 1G -R -c 172.20.0.110 --json' >>iperf3results/udp_server2client.json

docker compose exec --env RUST_LOG=info -it client /bin/sh -c 'iperf3 -t 30 -O 1 -u -b 1G -c 172.20.0.110 --json' >>iperf3results/udp_client2server.json
