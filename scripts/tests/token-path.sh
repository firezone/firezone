...
sudo "$BINARY_NAME" --check && exit 1

# Pass if we use the env var
sudo FIREZONE_TOKEN="$TOKEN" "$BINARY_NAME" --check

# Fails because passing tokens as CLI args is not allowed anymore
sudo "$BINARY_NAME" --check --token "$TOKEN" && exit 1
...
sudo "$BINARY_NAME" --check --token-path "$TOKEN_PATH"
