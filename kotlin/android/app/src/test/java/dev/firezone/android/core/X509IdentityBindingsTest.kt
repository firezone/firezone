...
import uniffi.x509identity.ParsedCertificate
import uniffi.x509identity.DetailField
import uniffi.x509identity.parseClientCertificate
import kotlin.reflect.KFunction

/**
 * Prototype smoke test for the x509identity UniFFI bindings: proves they are generated
...
class X509IdentityBindingsTest {
    ...
    @Test
    fun `x509identity bindings are reachable`() {
        assertEquals("parseClientCertificate", (parse as KFunction<*>).name)
    }

    @Test
    fun `x509identity records are plain kotlin`() {
