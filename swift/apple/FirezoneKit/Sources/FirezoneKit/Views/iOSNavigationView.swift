...
import SwiftUI
  ...
  struct IOSNavigationView<Content: View>: View {
    ...
    func startSession() {
      ...
      case .absent:
        signInButtonTapped()
      case .resolved(_, let actor):
        connectButtonTapped(as: actor)
      case .refused:
    ...
    func connectButtonTapped(as actor: X509ClaimedActor) {
      Task {
        do {
          try await store.connectWithCertificate(as: actor)
        } catch {
          Log.error(error)
