...
//
    ...
    SystemExtensionManagerProtocol
  {
    ...
    // Delegate callbacks arrive on main queue (specified in sendRequest).
    // Use nonisolated + MainActor.assumeIsolated to bridge the non-async delegate to our actor.
    ...
    nonisolated public func request(
      ...
      // OSSystemExtensionProperties isn't Sendable, so we capture what we need here
      ...
      // Extract extension info as Sendable tuples
      ...
      MainActor.assumeIsolated {
        ...
        let isCurrentVersionInstalled = enabledExtensions.contains { sysex in
        ...
        if let enabledExtension = enabledExtensions.first {
          Log.warning(
