...
//
  ...
  public enum StatusItemIntrospection {
    ...
    /// Asynchronously captures the status item once it becomes available.
    ///
    /// Call this after app launch to capture the NSStatusItem once SwiftUI
    /// has created the MenuBarExtra.
    public static func captureStatusItem(_ callback: @escaping @MainActor (NSStatusItem) -> Void) {
      Task { @MainActor in
        // Give SwiftUI time to create the MenuBarExtra
