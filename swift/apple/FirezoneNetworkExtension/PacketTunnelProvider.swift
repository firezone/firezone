...
class PacketTunnelProvider: NEPacketTunnelProvider {
  ...
  override func startTunnel(
    ...
    do {
      ...
      let adapter = Adapter(packetTunnelProvider: self)
      ...
      // Report success immediately - TUN setup will happen when we receive the first interface config
      Log.log("Adapter started successfully, reporting tunnel ready")
      completionHandler(nil)
...
extension PacketTunnelProvider {
  func onSetInterfaceConfig(
    ...
    // If this is the first config, set up TUN device after network settings are applied
    if !hasReceivedFirstConfig {
      hasReceivedFirstConfig = true
      networkSettings.apply { [weak self] in
        Log.log("Network settings applied, setting up TUN device...")
        self?.adapter?.setupTunDevice()
      }
    } else {
