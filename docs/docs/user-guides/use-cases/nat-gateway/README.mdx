...
title: NAT Gateway
sidebar_position: 3
description: Set a static IP for egress traffic with Firezone's WireGuard® based secure remote access platform. 
---
...
* Masking your device IP or proxying your source IP for privacy or
  security reasons.
* Restricting SSO authentication to users connected to the VPN
([see guide](whitelist-sso))
...
multiple end users, which may become labor-intensive to manage as the
access list grows.
