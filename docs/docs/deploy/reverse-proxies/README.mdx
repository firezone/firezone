...
:::info
Due to its speed, security, ease-of-use, and support of the latest
HTTP standards, Caddy (used by default for Docker-based deployments) is the
recommended proxy server to use with Firezone.
:::

:::warning
Using a custom reverse proxy is an advanced configuration. The default bundled
Nginx proxy (Omnibus-based installs) and Caddy (Docker-based installs) is
suitable for the vast majority of use cases and is recommended for most users.
There are important security risks if the Firezone reverse proxy
...
* [Apache](../reverse-proxies/apache)
* [Traefik](../reverse-proxies/traefik)
* [HAProxy](../reverse-proxies/haproxy)
