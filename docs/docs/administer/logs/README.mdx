...
sidebar_position: 8
description:
     Firezone's WireGuard®-based remote access platform stores debug logs and
...
Each container generates and stores debug logs to a JSON file on the host
...
containers. Note, `docker compose` commands need to run in the Firezone
