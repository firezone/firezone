...
title: Troubleshoot
sidebar_position: 6
description: Troubleshoot common connectivity and configuration issues with Firezone's WireGuard®-based remote access platform.
---
...
Each container stores logs as a JSON file on the host machine. These can be shown through the
`docker logs {CONTAINER}` command. Log files are found at
`var/lib/docker/containers/{CONTAINER_ID}/{CONTAINER_ID}-json.log`.

See [logs](../logs) for additional details.
...
If you are locked out of the admin account for your Firezone instance
you can re-create your admin user. 

<Tabs>
<TabItem value="docker" label="Docker">

Run the following command in the
home directory of your Firezone installation. This is where you can
find the `docker-compose.yml` and `.env` files.

```shell
docker compose exec firezone bin/create-or-reset-admin
```

This will reset the password on admin user using the default password
found in the `.env` environment file.
...
For production deployments, we recommend adding a TOTP-based second factor to admin accounts.
If you promoted an account authenticated through an identity provider, you can consider
disabling local authentication for additional security.

If errors occur with your identity provider integration, you can be locked out of
access to the admin portal. You can un the following query on Firezone's Postgres database
to re-enable local authentication.

```SQL
UPDATE configurations SET local_auth_enabled = 't'
```
