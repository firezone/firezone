...
Firezone 0.6 introduces **Docker support**, a REST API, SAML 2.0 authentication,
revamped user provisioning logic, and a slew of minor improvements and bugfixes.

### Migrate to Docker

Docker is now the preferred way to deploy and manage Firezone. See the [migration
guide](migrate) to migrate today. In most cases this can be done in a few minutes
...
at runtime. Check the [REST API reference](../reference/REST API/) for more information.
