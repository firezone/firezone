defmodule FzHttp.TestHelpers do
  ...
  def create_rules(_) do
    ...
    {rules_with_users, users_and_devices} =
      ...
      |> Enum.map(fn num ->
        ...
        rule = AllowRulesFixtures.allow_rule(%{destination: destination, user_id: user.uuid})
        {rule, {user, device}}
      end)
      |> Enum.unzip()
    ...
    rule_without_device =
      AllowRulesFixtures.allow_rule(%{destination: destination, user_id: user.uuid})
  ...
  def create_rule_with_user_and_device(_) do
    user = UsersFixtures.user()
    rule = AllowRulesFixtures.allow_rule(%{user_id: user.uuid, destination: "10.20.30.0/24"})
  ...
  def create_rule_with_user(opts \\ %{}) do
    user = UsersFixtures.user()
    rule = AllowRulesFixtures.allow_rule(Map.merge(%{user_id: user.uuid}, opts))
