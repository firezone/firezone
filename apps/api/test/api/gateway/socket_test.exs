...
#   import API.Gateway.Socket, only: [id: 1]
#   alias API.Gateway.Socket
#   alias Domain.Auth
#   alias Domain.{SubjectFixtures, GatewaysFixtures}
...
#       subject = SubjectFixtures.create_subject()
#       existing_client = GatewaysFixtures.create_client(subject: subject)
#       token = Auth.create_auth_token(subject)
...
#       assert {:ok, socket} = connect(Socket, attrs, @connect_info)
#       assert client = Repo.one(Domain.Gateways.Gateway)
...
#       socket = socket(API.Gateway.Socket, "", %{client: client})
