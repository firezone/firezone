defmodule Web.SettingLive.SecurityTest do
  use Web.ConnCase, async: true
  alias Domain.ConfigFixtures
  alias Web.SettingLive.Security
  ...
  describe "authenticated mount" do
    ...
    test "selects the chosen option", %{admin_conn: conn} do
      ...
      Domain.Config.put_config!(:vpn_session_duration, 3_600)
  ...
  describe "toggles" do
    setup %{conf_key: key, conf_val: val} do
      Domain.Config.put_config!(key, val)
    ...
    for {key, val} <- [
      ...
      test "toggle #{key} when value in db is true", %{admin_conn: conn, path: path} do
        ...
        assert Domain.Config.fetch_config!(unquote(key)) == false
      end
    end
    ...
    for {key, val} <- [
      ...
      test "toggle #{key} when value in db is false", %{admin_conn: conn, path: path} do
        ...
        view |> element("input[phx-value-config=#{unquote(key)}]") |> render_click()
        assert Domain.Config.fetch_config!(unquote(key)) == true
      end
    end
  end
  ...
  describe "oidc configuration" do
    ...
    test "validate", %{view: view} do
      ...
      assert %Domain.Config.Configuration.OpenIDConnectProvider{
               id: "test",
               label: "updated",
               ...
               redirect_uri: nil,
               auto_create_users: false
             } in Domain.Config.fetch_config!(:openid_connect_providers)
    end
    ...
    test "delete", %{view: view} do
      ...
      openid_connect_providers = Domain.Config.fetch_config!(:openid_connect_providers)
  ...
  describe "saml configuration" do
    ...
    test "click add button", %{view: view} do
      ...
      saml_identity_providers = Domain.Config.fetch_config!(:saml_identity_providers)
      ...
      assert %Domain.Config.Configuration.SAMLIdentityProvider{
               ...
               base_url: "#{Domain.Config.fetch_env!(:web, :external_url)}auth/saml",
               id: "FAKEID",
               label: "FOO",
               metadata: attrs["metadata"],
               sign_metadata: true,
    ...
    test "edit", %{view: view} do
      ...
      assert saml_identity_provider =
               Domain.Config.fetch_config!(:saml_identity_providers)
    ...
    test "validate", %{view: view} do
      ...
      assert %Domain.Config.Configuration.SAMLIdentityProvider{
               ...
               signed_assertion_in_resp: true,
               signed_envelopes_in_resp: true
             } in Domain.Config.fetch_config!(:saml_identity_providers)
    end
