defmodule Web.Plug.PathPrefixTest do
  use Web.ConnCase, async: true
  import Plug.Test
  import Web.Plug.PathPrefix
  ...
  describe "call/2" do
    test "does nothing when path prefix is not configured" do
      Domain.Config.put_env_override(:web, :path_prefix, nil)
      conn = conn(:get, "/")
      assert call(conn, []) == conn

      Domain.Config.put_env_override(:web, :path_prefix, "/")
      conn = conn(:get, "/foo")
      assert call(conn, []) == conn
    end

    test "removes prefix from conn.request_path" do
      Domain.Config.put_env_override(:web, :path_prefix, "/vpn/")
