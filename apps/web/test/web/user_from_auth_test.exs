defmodule Web.UserFromAuthTest do
  ...
  describe "find_or_create/2 via OIDC with auto create enabled" do
    test "sign in creates user", %{email: email} do
      Domain.ConfigFixtures.start_openid_providers(["google"], %{
  ...
  describe "find_or_create/2 via SAML with auto create enabled" do
    @tag config: [Domain.ConfigFixtures.saml_identity_providers_attrs()]
    test "sign in creates user", %{config: config, email: email} do
      Domain.Config.put_config!(:saml_identity_providers, config)
  ...
  describe "find_or_create/2 via SAML with auto create disabled" do
    @tag config: [
           Domain.ConfigFixtures.saml_identity_providers_attrs(%{"auto_create_users" => false})
         ]
