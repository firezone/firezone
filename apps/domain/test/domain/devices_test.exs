defmodule Domain.DevicesTest do
  ...
  alias Domain.{UsersFixtures, SubjectFixtures, DevicesFixtures}
  ...
  describe "new_device/0" do
    test "returns changeset with default values" do
      assert %Ecto.Changeset{data: %Domain.Devices.Device{}} = changeset = new_device()
  ...
  describe "create_device_for_user/3" do
    ...
    test "ignores configuration attrs when there are no configure permission", %{
      ...
      Domain.Config.put_system_env_override(:allow_unprivileged_device_configuration, false)
      Domain.Config.put_env_override(:max_devices_per_user, 100)
    ...
    test "prevents creating more than max_devices_per_user", %{
      ...
      Domain.Config.put_env_override(:max_devices_per_user, 1)
    ...
    test "returns error when device IP can't be assigned due to CIDR pool exhaustion", %{
      ...
      {:ok, cidr} = Domain.Types.CIDR.cast("10.3.2.0/30")
  ...
  describe "to_peer_list/0" do
    ...
    test "does not render peers for users with expired VPN session" do
      Domain.Config.put_system_env_override(:vpn_session_duration, 1)
